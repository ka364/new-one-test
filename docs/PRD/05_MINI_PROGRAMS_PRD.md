...
|---------|----------|---------|-------------|
| برامج ضارة أو احتيالية | منخفض | عالي | مراجعة أمنية صارمة، sandbox معزول، مراقبة مستمرة |
| أداء سيء يؤثر على التطبيق الأم | متوسط | عالي | حدود موارد صارمة، إيقاف تلقائي للبرامج البطيئة |
| انتهاك الخصوصية | منخفض | عالي | صلاحيات محدودة، موافقة صريحة، تدقيق دوري |
| محتوى غير لائق | متوسط | متوسط | مراجعة محتوى، بلاغات المستخدمين، إزالة فورية |
| عدم جذب مطورين | متوسط | متوسط | حوافز مالية، توثيق ممتاز، دعم فني مميز |
| تجربة مستخدم سيئة | متوسط | متوسط | معايير UX إلزامية، اختبار قبل النشر |

---

## 12. خطة الطوارئ (Contingency Plan)

### 12.1 ماذا لو اكتُشف برنامج ضار؟
```
السيناريو: برنامج مصغر يسرق بيانات أو يضر بالمستخدمين
الإجراءات:
1. إيقاف فوري للبرنامج
2. إشعار جميع المستخدمين المتأثرين
3. تحقيق أمني شامل
4. إيقاف حساب المطور + إجراء قانوني
5. تقرير شفافية للمستخدمين
6. تحسين نظام المراجعة
```

### 12.2 ماذا لو انهار برنامج شائع؟
```
السيناريو: برنامج مصغر مستخدم من آلاف توقف عن العمل
الإجراءات:
1. إشعار المطور فوراً
2. عرض المساعدة التقنية
3. إذا لم يُصلح خلال 24 ساعة: إشعار المستخدمين
4. تقديم بدائل مشابهة
5. rollback لآخر نسخة مستقرة إن أمكن
```

### 12.3 ماذا لو انسحب تاجر/مطور كبير؟
```
السيناريو: مطور لبرنامج شهير يريد الانسحاب
الإجراءات:
1. محاولة الاحتفاظ (حوافز، دعم إضافي)
2. فترة إشعار 30 يوم للمستخدمين
3. تصدير بيانات المستخدمين
4. اقتراح بدائل للمستخدمين
5. دراسة أسباب الانسحاب للتحسين
```
...
|---------|---------|-------|--------------|
| **تكلفة اكتساب مطور** | تكلفة جلب مطور جديد | < 500 ج.م | تكاليف التسويق ÷ مطورين جدد |
| **معدل نجاح المراجعة** | برامج تُقبل من أول مرة | > 70% | مقبول ÷ إجمالي المقدم |
| **وقت المراجعة** | من التقديم للقرار | < 48 ساعة | متوسط الوقت |
| **معدل التحويل** | مستخدمون يشترون | > 5% | مشترين ÷ مستخدمين |
| **GMV لكل برنامج** | حجم المبيعات | > 10,000 ج.م/شهر | إجمالي GMV ÷ برامج نشطة |
| **رضا المطورين** | تقييم المطورين للمنصة | > 4.0/5 | استطلاع دوري |
