...
export async function migrateProductImages(
  ...
  const results = await Promise.all(
    googleDriveUrls.map((url, index) => migrateImageToS3(url, productModelCode, index))
  );
...
export async function batchMigrateImages(
  ...
  for (let i = 0; i < migrations.length; i += concurrency) {
    ...
    // Log progress
    console.log(
      `[Image Migration] Progress: ${Math.min(i + concurrency, migrations.length)}/${migrations.length}`
    );
  }
