...
export default function BudgetManagement() {
    ...
    const departments: { [key: string]: Department } = {
        hr: {
            ...
            totalSpent: 950000,
            items: [
                { id: "1", category: "الرواتب", allocated: 800000, spent: 800000, percentage: 100, status: "on_track" },
                { id: "2", category: "التدريب", allocated: 200000, spent: 120000, percentage: 60, status: "on_track" },
                { id: "3", category: "المزايا", allocated: 150000, spent: 30000, percentage: 20, status: "on_track" },
                { id: "4", category: "التوظيف", allocated: 50000, spent: 0, percentage: 0, status: "on_track" },
            ],
        },
        operations: {
            id: "operations",
            ...
            totalSpent: 2100000,
            items: [
                { id: "1", category: "المعدات", allocated: 1000000, spent: 900000, percentage: 90, status: "on_track" },
                { id: "2", category: "الصيانة", allocated: 800000, spent: 750000, percentage: 93.75, status: "warning" },
                { id: "3", category: "الإمدادات", allocated: 500000, spent: 350000, percentage: 70, status: "on_track" },
                { id: "4", category: "الطاقة", allocated: 200000, spent: 100000, percentage: 50, status: "on_track" },
            ],
        },
        marketing: {
            id: "marketing",
            ...
            totalSpent: 700000,
            items: [
                { id: "1", category: "الإعلانات الرقمية", allocated: 400000, spent: 350000, percentage: 87.5, status: "on_track" },
                { id: "2", category: "المحتوى", allocated: 200000, spent: 180000, percentage: 90, status: "on_track" },
                { id: "3", category: "الفعاليات", allocated: 150000, spent: 150000, percentage: 100, status: "warning" },
                { id: "4", category: "البحث", allocated: 50000, spent: 20000, percentage: 40, status: "on_track" },
            ],
        },
        it: {
            id: "it",
            ...
            totalSpent: 1350000,
            items: [
                { id: "1", category: "البرامج", allocated: 600000, spent: 600000, percentage: 100, status: "on_track" },
                { id: "2", category: "البنية التحتية", allocated: 500000, spent: 450000, percentage: 90, status: "on_track" },
                { id: "3", category: "الأمان", allocated: 300000, spent: 250000, percentage: 83.33, status: "on_track" },
                { id: "4", category: "الدعم", allocated: 100000, spent: 50000, percentage: 50, status: "on_track" },
            ],
        },
    };
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b">
                                <tr>
                                    <th className="px-4 py-3 text-right text-sm font-semibold text-gray-700">البند</th>
                                    <th className="px-4 py-3 text-right text-sm font-semibold text-gray-700">المخصص</th>
                                    ...
                                    <th className="px-4 py-3 text-right text-sm font-semibold text-gray-700">النسبة</th>
                            ...
                            <tbody>
                                {currentDept.items.map((item) => (
                                    <tr key={item.id} className="border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-600">{(item.allocated / 1000).toFixed(0)}K</td>
                                        <td className="px-4 py-3 text-sm text-green-600 font-medium">{(item.spent / 1000).toFixed(0)}K</td>
                                        <td className="px-4 py-3 text-sm text-gray-600">{((item.allocated - item.spent) / 1000).toFixed(0)}K</td>
                                        <td className="px-4 py-3 text-sm">
                                            <div className="flex items-center gap-2">
                                                <div className="w-16 bg-gray-200 rounded-full h-2">
                                                    <div
                                                        className={`h-2 rounded-full transition-all ${item.status === "on_track"
                                                                ? "bg-green-500"
                                                                : item.status === "warning"
                                                                    ? "bg-orange-500"
                                                                    : "bg-red-500"
                                                ...
                                                <span className="text-xs font-semibold text-gray-700 w-8 text-right">{item.percentage.toFixed(0)}%</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-sm">
                                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(item.status)}`}>
                                                {item.status === "on_track" ? "متابع" : item.status === "warning" ? "تحذير" : "تجاوز"}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
