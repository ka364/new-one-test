...
from backend.core.error_handler import handle_endpoint_errors, ValidationException, ResourceNotFoundException
...
class ModuleInitRequest(BaseModel):
    ...
    module_name: str = Field(..., min_length=1, description="Module display name")
    organism: str = Field(..., min_length=1, description="Organism inspiration")
    phase: str = Field(..., min_length=1, description="HADER lifecycle phase")
...
async def submit_deliverable(request: DeliverableSubmitRequest):
    ...
    if not success:
        raise ResourceNotFoundException("Module or deliverable")
...
async def validate_module(module_id: str):
    ...
    if not module_id or not module_id.strip():
        raise ValidationException("module_id is required")
...
async def get_module_status(module_id: str):
    ...
    if state:
        ...
        raise ResourceNotFoundException("Module")
