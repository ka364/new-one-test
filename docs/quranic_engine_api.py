...
class QuranicEngine:
    ...
    def search_logical(
        ...
        """
        بحث منطقي (AND/OR) على مصطلحات متعددة.
        - query: عدة كلمات مفصولة بمسافة
        - operator: 'AND' أو 'OR'
        - by_root: إذا True يستخدم البحث الجذري، وإلا البحث الحرفي لكل كلمة
        """
        terms = query.split()
        # لكل مصطلح، نأتي بمجموعة من النتائج
        term_sets = []
        for t in terms:
            if by_root:
                root = self.stemmer.stem(re.sub(r'[^\w\s]', '', t))
                verses = self.search_by_root(root)
            else:
                verses = self.search_exact(t)
            # نضع مجموعة مفاتيح فريدة لكل مرجع
        ...
        # نعيد قائمة القواميس
        return [
            {'surah_number': s, 'surah_name': n, 'verse_number': v, 'text': txt}
            for (s, v, txt), n in [((s, v, txt), self.get_surah(s)['name']) for s, v, txt in common]
        ]
...
def api_search(q: str, mode: str = 'root', operator: str = 'AND'):
    """
    بحث عبر HTTP:
      - q: نص الاستعلام (مصطلحات مفصولة بمسافة)
      - mode: 'root' أو 'exact'
      - operator: 'AND' أو 'OR'
    """
