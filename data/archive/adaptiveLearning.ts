...
export async function trackUserBehavior(
  ...
  const behavior: InsertUserBehavior = {
    userId,
    actionType,
...
async function updateOrCreateTaskPattern(
  ...
  if (existing.length > 0) {
    ...
    // إنشاء نمط جديد
    const pattern: InsertTaskPattern = {
      userId,
      taskType,
...
export async function getUserDynamicIcons(userId: number) {
  ...
  return await db
    .select()
    .from(dynamicIcons)
    .where(and(eq(dynamicIcons.userId, userId), eq(dynamicIcons.isVisible, true)))
