...
const createAutomationSchema = z.object({
  ...
  description: z.string().optional(),
  trigger: z.enum(["keyword", "first_message", "order_status", "cart_abandoned", "payment_received", "delivery_update", "review_request", "custom"]),
...
export const whatsappCommerceRouter = router({
  ...
  getCart: publicProcedure
    ...
    .query(async ({ input }) => {
      const [cart] = await db
        ...
        .from(whatsappCarts)
        .where(and(
          eq(whatsappCarts.phoneNumber, input.phoneNumber),
  ...
  addToCart: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!cart) {
        [cart] = await db.insert(whatsappCarts).values({
      ...
      const [cartItem] = await db.insert(whatsappCartItems).values({
        cartId: cart.id,
  ...
  getConversations: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const conversations = await db
        ...
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(
          desc(whatsappConversations.isPinned),
  ...
  createAutomation: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const [automation] = await db.insert(whatsappAutomations).values({
        name: input.name,
        nameAr: input.nameAr,
        description: input.description,
  ...
  createQuickReply: publicProcedure
    .input(z.object({
      shortcut: z.string().min(2).regex(/^\/\w+$/),
    ...
    .mutation(async ({ input }) => {
      const [reply] = await db.insert(whatsappQuickReplies).values({
  ...
  getAnalytics: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [cartStats] = await db
        ...
        .from(whatsappCarts)
        .where(and(
          gte(whatsappCarts.createdAt, startDate),
          lte(whatsappCarts.createdAt, endDate)
        ));
      ...
      return {
        ...
        messages: {
          ...
          deliveryRate: messageStats.sent ? ((Number(messageStats.delivered) / Number(messageStats.sent)) * 100).toFixed(1) : "0",
          readRate: messageStats.sent ? ((Number(messageStats.read) / Number(messageStats.sent)) * 100).toFixed(1) : "0",
        },
        carts: {
          total: Number(cartStats.total) || 0,
          converted: Number(cartStats.converted) || 0,
          abandoned: Number(cartStats.abandoned) || 0,
          conversionRate: cartStats.total ? ((Number(cartStats.converted) / Number(cartStats.total)) * 100).toFixed(1) : "0",
