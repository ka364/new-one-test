# Product Requirements Document (PRD)
# نظام مجموعات الشراء المجتمعية - Community Buying Groups

---

## 📋 معلومات المستند

| البند | القيمة |
|-------|--------|
| **رقم المستند** | PRD-HADEROS-CBG-003 |
| **الإصدار** | 1.0 |
| **تاريخ الإنشاء** | 2 يناير 2026 |
| **الحالة** | مسودة للمراجعة |

---

## 1. نظرة عامة (Overview)

### 1.1 ملخص تنفيذي
نظام مجموعات الشراء المجتمعية يمكّن السكان المحليين من تشكيل مجموعات شراء دائمة في أحيائهم، بقيادة شخص محلي موثوق (Community Leader) يجمع الطلبات ويشرف على توزيعها، مقابل عمولة.

### 1.2 الفرق عن الشراء الجماعي (Group Buying)

| الميزة | الشراء الجماعي | مجموعات الشراء المجتمعية |
|--------|----------------|---------------------------|
| **المدة** | عرض مؤقت (24-72 ساعة) | مجموعة دائمة |
| **القيادة** | أي مستخدم | قائد معتمد ومُعرّف |
| **المنتجات** | منتج واحد | كتالوج كامل |
| **التوصيل** | فردي أو مجمع | مجمع دائماً |
| **الدفع** | فوري | أسبوعي/شهري |
| **العلاقة** | معاملة واحدة | علاقة مستمرة |

### 1.3 لماذا مهم لمصر؟

```
🏘️ ثقافة الجيرة القوية في مصر
   └── الجيران يثقون ببعضهم للشراء نيابة عنهم

💰 توفير مصاريف الشحن
   └── توصيل واحد لـ 20-50 طلب بدلاً من توصيلات منفصلة

👤 فرصة دخل للقادة المحليين
   └── 5-10% عمولة على كل طلب

🚚 حل مشكلة "غير موجود في البيت"
   └── القائد دائماً متاح لاستلام الطلبات
```

### 1.4 الجمهور المستهدف

**القادة المحتملون (Community Leaders):**
| الشريحة | الوصف | المزايا |
|---------|-------|---------|
| ربات البيوت | متواجدات دائماً، شبكة علاقات قوية | وقت متاح، ثقة الجيران |
| البوابين | يعرفون كل السكان، متواجدون | موقع مثالي، معرفة الجميع |
| أصحاب المحلات | لديهم مكان للتخزين | مساحة، خبرة تجارية |
| المتقاعدون | وقت فراغ، احترام المجتمع | خبرة، ثقة، تفرغ |
| طلاب الجامعات | نشطون، تقنيون | طاقة، فهم التكنولوجيا |

**الأعضاء المحتملون:**
| الشريحة | الاحتياج |
|---------|----------|
| الأسر العاملة | لا وقت للتسوق |
| كبار السن | صعوبة الخروج |
| الأمهات الجدد | مشغولات بالأطفال |
| سكان المجمعات | يريدون توفير الشحن |

---

## 2. المتطلبات الوظيفية (Functional Requirements)

### 2.1 نظام القادة (Community Leaders)

#### FR-CBG-001: تسجيل وتأهيل القائد
**الوصف:** عملية التقديم والموافقة على قادة المجموعات

**معايير القبول:**
```typescript
interface LeaderQualifications {
  // المعايير الأساسية
  basic: {
    minAge: 21;                      // الحد الأدنى للعمر
    hasValidId: boolean;             // بطاقة هوية صالحة
    hasSmartphone: boolean;          // هاتف ذكي
    residencyProof: boolean;         // إثبات سكن في المنطقة
    minResidencyMonths: 12;          // 12 شهر على الأقل في المنطقة
  };

  // المعايير التفضيلية
  preferred: {
    previousLeadershipRole: boolean; // خبرة قيادية سابقة
    businessExperience: boolean;     // خبرة تجارية
    largeNetwork: boolean;           // شبكة علاقات واسعة
    ownsCar: boolean;                // يمتلك سيارة (للتوزيع)
    hasStorageSpace: boolean;        // مساحة للتخزين
  };

  // نقاط التقييم
  scoring: {
    maxScore: 100;
    passScore: 60;
    weights: {
      residency: 20;
      network: 25;
      reliability: 25;
      availability: 15;
      resources: 15;
    };
  };
}
```

**خطوات التسجيل:**
```
1. تقديم الطلب
   ├── البيانات الشخصية
   ├── صور الهوية
   ├── إثبات السكن
   └── معلومات الاتصال

2. التحقق الأولي
   ├── التحقق من الهوية (KYC)
   ├── التحقق من العنوان
   └── فحص السجل (إن أمكن)

3. المقابلة والتدريب
   ├── مقابلة فيديو (15 دقيقة)
   ├── دورة تدريبية أونلاين (2 ساعة)
   └── اختبار قصير

4. الفترة التجريبية
   ├── 30 يوم كقائد مبتدئ
   ├── حد أقصى 10 أعضاء
   └── مراقبة الأداء

5. الاعتماد النهائي
   ├── ترقية لقائد معتمد
   ├── رفع الحدود
   └── الحصول على شارة
```

---

#### FR-CBG-002: لوحة تحكم القائد (Leader Dashboard)
**الوصف:** واجهة شاملة لإدارة المجموعة

```
┌─────────────────────────────────────────────────────────────────────┐
│  ☰  HADEROS Community Leader                          أحمد محمد 👤  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  🏘️ مجموعة عمارة النور - المعادي                                    │
│  ⭐ 4.9 (156 تقييم)  |  👥 42 عضو  |  📦 1,250 طلب                  │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  💰 أرباحك هذا الشهر                                          │ │
│  │                                                               │ │
│  │         2,450 ج.م                                             │ │
│  │         ────────────                                          │ │
│  │         ↑ 15% عن الشهر الماضي                                 │ │
│  │                                                               │ │
│  │  [  سحب الأرباح  ]                                            │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  📊 ملخص هذا الأسبوع                                                │
│  ┌─────────────┬─────────────┬─────────────┬─────────────┐        │
│  │  طلبات     │  إجمالي    │  عمولتك    │  للتوزيع   │        │
│  │    32      │  8,500     │   425      │    12      │        │
│  │  طلب       │  ج.م       │   ج.م      │   طلب      │        │
│  └─────────────┴─────────────┴─────────────┴─────────────┘        │
│                                                                     │
│  📅 جولة الطلب الحالية                                              │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  🟢 مفتوحة الآن                                               │ │
│  │  ⏰ تنتهي: الأربعاء 8 يناير، 8:00 م                            │ │
│  │  📦 التوزيع: الجمعة 10 يناير، 4:00 م - 8:00 م                  │ │
│  │                                                               │ │
│  │  الطلبات: 18  |  الإجمالي: 4,200 ج.م                          │ │
│  │  [████████████░░░░░░░░] 60%                                   │ │
│  │                                                               │ │
│  │  [ إغلاق الجولة مبكراً ]   [ تمديد الموعد ]                   │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  🔔 إشعارات (3)                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  • عضو جديد: فاطمة أحمد (شقة 12) طلبت الانضمام                │ │
│  │  • طلب جديد: محمد سعيد طلب 5 منتجات (350 ج.م)                 │ │
│  │  • تذكير: موعد التوزيع غداً الساعة 4 م                        │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  ⚡ إجراءات سريعة                                                   │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐  │
│  │ 📢 إرسال   │ │ 📦 طلبات   │ │ 👥 الأعضاء │ │ ⚙️ إعدادات │  │
│  │   تنبيه   │ │   جديدة   │ │           │ │           │  │
│  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘  │
│                                                                     │
├─────────────────────────────────────────────────────────────────────┤
│   [🏠]        [📦]        [👥]        [💰]        [📊]              │
│  الرئيسية    الطلبات     الأعضاء     الأرباح    التقارير           │
└─────────────────────────────────────────────────────────────────────┘
```

---

#### FR-CBG-003: إدارة جولات الطلب (Order Rounds)
**الوصف:** نظام جولات الطلب الدورية

```typescript
interface OrderRound {
  id: number;
  groupId: number;
  roundCode: string;           // "R-2026-W02" (الأسبوع 2 من 2026)

  // التوقيت
  timing: {
    orderStart: Date;          // بداية قبول الطلبات
    orderDeadline: Date;       // آخر موعد للطلب
    collectionDate: Date;      // تاريخ التوزيع
    collectionTimeFrom: string; // "16:00"
    collectionTimeTo: string;   // "20:00"
  };

  // الإعدادات
  settings: {
    minOrderAmount: number;    // الحد الأدنى للطلب
    maxOrdersCount: number;    // الحد الأقصى للطلبات
    allowLateOrders: boolean;  // السماح بالطلبات المتأخرة
    lateOrderFee: number;      // رسوم الطلب المتأخر
  };

  // الإحصائيات
  stats: {
    ordersCount: number;
    totalAmount: number;
    uniqueMembers: number;
    avgOrderValue: number;
  };

  // الحالة
  status: 'scheduled' | 'open' | 'closed' | 'processing' | 'ready' | 'distributed' | 'completed';
}

// دورة حياة الجولة
const roundLifecycle = {
  scheduled: {
    description: 'جولة مجدولة لم تبدأ بعد',
    nextStatus: 'open',
    trigger: 'وصول وقت البداية'
  },
  open: {
    description: 'مفتوحة لاستقبال الطلبات',
    nextStatus: 'closed',
    trigger: 'وصول الموعد النهائي أو إغلاق يدوي'
  },
  closed: {
    description: 'مغلقة، لا طلبات جديدة',
    nextStatus: 'processing',
    trigger: 'بدء معالجة الطلبات'
  },
  processing: {
    description: 'جاري تجهيز الطلبات',
    nextStatus: 'ready',
    trigger: 'وصول الشحنة للقائد'
  },
  ready: {
    description: 'جاهزة للتوزيع',
    nextStatus: 'distributed',
    trigger: 'بدء التوزيع'
  },
  distributed: {
    description: 'جاري التوزيع',
    nextStatus: 'completed',
    trigger: 'اكتمال جميع التسليمات'
  },
  completed: {
    description: 'مكتملة',
    nextStatus: null,
    trigger: null
  }
};
```

---

### 2.2 نظام العضوية

#### FR-CBG-004: انضمام الأعضاء
**الوصف:** عملية طلب الانضمام لمجموعة

```typescript
interface MembershipRequest {
  userId: number;
  groupId: number;

  // معلومات السكن
  residence: {
    buildingName: string;       // اسم العمارة
    buildingNumber: string;     // رقم العمارة
    floorNumber: string;        // الدور
    apartmentNumber: string;    // رقم الشقة
    landmark: string;           // علامة مميزة
  };

  // التفضيلات
  preferences: {
    preferredCollectionTime: 'morning' | 'afternoon' | 'evening';
    canCollectForNeighbors: boolean;  // يمكنه استلام للجيران
    preferredPaymentMethod: 'cash' | 'wallet' | 'card';
    communicationPreference: 'whatsapp' | 'sms' | 'app';
  };

  // الموافقة
  agreements: {
    termsAccepted: boolean;
    privacyAccepted: boolean;
    paymentTermsAccepted: boolean;
  };
}

// مستويات العضوية
enum MembershipLevel {
  NEW = 'new',           // عضو جديد (أول 3 طلبات)
  REGULAR = 'regular',   // عضو عادي
  TRUSTED = 'trusted',   // عضو موثوق (10+ طلبات، 4.5+ تقييم)
  VIP = 'vip'           // عضو VIP (50+ طلب، يساعد في التوزيع)
}

// امتيازات كل مستوى
const membershipBenefits = {
  new: {
    orderLimit: 500,           // حد الطلب 500 ج.م
    paymentOptions: ['cash'],   // نقدي فقط
    canOrderLate: false
  },
  regular: {
    orderLimit: 2000,
    paymentOptions: ['cash', 'wallet'],
    canOrderLate: false
  },
  trusted: {
    orderLimit: 5000,
    paymentOptions: ['cash', 'wallet', 'card', 'credit'],
    canOrderLate: true,
    lateOrderDiscount: 0.5    // خصم 50% على رسوم التأخير
  },
  vip: {
    orderLimit: 10000,
    paymentOptions: ['cash', 'wallet', 'card', 'credit'],
    canOrderLate: true,
    lateOrderFee: 0,           // بدون رسوم تأخير
    priorityCollection: true,  // أولوية في الاستلام
    helperBonus: 50            // 50 ج.م عند المساعدة في التوزيع
  }
};
```

---

#### FR-CBG-005: نظام الطلب للأعضاء
**الوصف:** واجهة طلب المنتجات للأعضاء

```
┌─────────────────────────────────────────────────────────────────────┐
│  ← رجوع              مجموعة عمارة النور                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  📅 جولة الطلب الحالية                                              │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  ⏰ آخر موعد: الأربعاء 8 يناير، 8:00 م                        │ │
│  │  📦 التوزيع: الجمعة 10 يناير                                  │ │
│  │  💰 الحد الأدنى للطلب: 50 ج.م                                 │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  🔍 ابحث عن منتج...                                                 │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  [                                            ] [🔍]          │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  📂 الفئات                                                          │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐      │
│  │ 🥬     │ │ 🥛     │ │ 🧹     │ │ 👶     │ │ ⚡     │      │
│  │ خضروات │ │ ألبان  │ │ تنظيف │ │ أطفال  │ │ عروض   │      │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘      │
│                                                                     │
│  🔥 الأكثر طلباً في مجموعتك                                         │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  [صورة] زيت عافية 2 لتر        [صورة] أرز بسمتي 5 كجم        │ │
│  │         85 ج.م                        120 ج.م                │ │
│  │         ⭐ طلبه 15 عضو               ⭐ طلبه 12 عضو          │ │
│  │         [أضف للسلة]                   [أضف للسلة]            │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  📦 طلبك الحالي                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  3 منتجات  •  الإجمالي: 280 ج.م                               │ │
│  │                                                               │ │
│  │  [        عرض السلة وإتمام الطلب        ]                    │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

### 2.3 نظام التوزيع

#### FR-CBG-006: إدارة التوزيع
**الوصف:** نظام توزيع الطلبات على الأعضاء

```typescript
interface DistributionSession {
  id: number;
  roundId: number;
  leaderId: number;

  // التوقيت
  scheduledDate: Date;
  scheduledTimeFrom: string;
  scheduledTimeTo: string;
  actualStartTime?: Date;
  actualEndTime?: Date;

  // الموقع
  location: {
    type: 'leader_home' | 'building_entrance' | 'meeting_point' | 'custom';
    address: string;
    coordinates?: { lat: number; lng: number };
    instructions: string;
  };

  // الإحصائيات
  stats: {
    totalOrders: number;
    collectedOrders: number;
    pendingOrders: number;
    noShowOrders: number;
    totalCollected: number;
    totalPending: number;
  };
}

// حالات الطلب في التوزيع
enum OrderCollectionStatus {
  PENDING = 'pending',           // في انتظار الاستلام
  NOTIFIED = 'notified',         // تم إخطار العضو
  READY = 'ready',               // العضو في الطريق
  COLLECTED = 'collected',       // تم الاستلام
  DELEGATE = 'delegate',         // سيستلمه شخص آخر
  NO_SHOW = 'no_show',           // لم يحضر
  RESCHEDULED = 'rescheduled'    // مؤجل
}
```

**شاشة التوزيع للقائد:**
```
┌─────────────────────────────────────────────────────────────────────┐
│  ← رجوع           توزيع جولة الأسبوع 2                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  📊 ملخص التوزيع                                                    │
│  ┌─────────────┬─────────────┬─────────────┬─────────────┐        │
│  │  الإجمالي  │  تم استلامه │  في انتظار │  لم يحضر   │        │
│  │    32      │     24     │     6      │     2      │        │
│  │   طلب     │    طلب     │    طلب    │    طلب     │        │
│  └─────────────┴─────────────┴─────────────┴─────────────┘        │
│                                                                     │
│  💰 المبالغ                                                         │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  المطلوب تحصيله: 8,500 ج.م                                    │ │
│  │  تم تحصيله: 6,200 ج.م                                         │ │
│  │  متبقي: 2,300 ج.م                                             │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  📋 قائمة الطلبات                                                   │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  🔍 بحث...                     [الكل ▼] [ترتيب: الشقة ▼]      │ │
│  ├───────────────────────────────────────────────────────────────┤ │
│  │                                                               │ │
│  │  ✅ فاطمة أحمد - شقة 12، الدور 3                              │ │
│  │     📦 5 منتجات  •  💰 350 ج.م  •  ✓ مدفوع                    │ │
│  │     ⏰ استلمت 4:15 م                                         │ │
│  │                                                               │ │
│  │  ───────────────────────────────────────────────────────────  │ │
│  │                                                               │ │
│  │  ⏳ محمد سعيد - شقة 8، الدور 2                                 │ │
│  │     📦 3 منتجات  •  💰 180 ج.م  •  💵 نقدي                    │ │
│  │     [📞 اتصال]  [💬 رسالة]  [✓ تم الاستلام]                  │ │
│  │                                                               │ │
│  │  ───────────────────────────────────────────────────────────  │ │
│  │                                                               │ │
│  │  ⏳ سارة علي - شقة 15، الدور 4                                 │ │
│  │     📦 8 منتجات  •  💰 520 ج.م  •  💳 مدفوع                   │ │
│  │     📝 ملاحظة: ابنها سيستلم                                   │ │
│  │     [📞 اتصال]  [💬 رسالة]  [✓ تم الاستلام]                  │ │
│  │                                                               │ │
│  │  ───────────────────────────────────────────────────────────  │ │
│  │                                                               │ │
│  │  ❌ أحمد حسن - شقة 3، الدور 1                                  │ │
│  │     📦 2 منتجات  •  💰 95 ج.م  •  💵 نقدي                     │ │
│  │     ⚠️ لم يحضر - تم الاتصال 3 مرات                            │ │
│  │     [📞 اتصال]  [📅 إعادة جدولة]  [❌ إلغاء]                  │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  [ 📢 إرسال تذكير للجميع ]   [ ✓ إنهاء التوزيع ]             │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

### 2.4 نظام العمولات والمكافآت

#### FR-CBG-007: حساب عمولات القائد
**الوصف:** نظام حساب ودفع عمولات القادة

```typescript
interface LeaderCommission {
  leaderId: number;
  roundId: number;

  // العمولة الأساسية
  baseCommission: {
    roundTotal: number;          // إجمالي الجولة
    commissionRate: number;      // نسبة العمولة (5-10%)
    baseAmount: number;          // المبلغ الأساسي
  };

  // المكافآت الإضافية
  bonuses: {
    newMembersBonus: number;     // مكافأة الأعضاء الجدد
    highVolumeBonus: number;     // مكافأة الحجم العالي
    perfectDistribution: number; // مكافأة التوزيع المثالي
    onTimeBonus: number;         // مكافأة الالتزام بالوقت
    ratingBonus: number;         // مكافأة التقييم الممتاز
  };

  // الخصومات
  deductions: {
    lateDistribution: number;    // خصم التأخير
    complaints: number;          // خصم الشكاوى
    refunds: number;             // خصم المرتجعات
  };

  // الإجمالي
  total: number;
  status: 'pending' | 'approved' | 'paid';
  paidAt?: Date;
}

// حساب العمولة
function calculateLeaderCommission(round: OrderRound, leader: Leader): LeaderCommission {
  // 1. العمولة الأساسية
  let baseRate = 0.05; // 5% أساسي

  // ترقية النسبة حسب الأداء
  if (leader.totalRounds > 50 && leader.avgRating >= 4.8) {
    baseRate = 0.08; // 8% للقادة المميزين
  } else if (leader.totalRounds > 20 && leader.avgRating >= 4.5) {
    baseRate = 0.07; // 7% للقادة المتقدمين
  } else if (leader.totalRounds > 10) {
    baseRate = 0.06; // 6% للقادة العاديين
  }

  const baseAmount = round.stats.totalAmount * baseRate;

  // 2. المكافآت
  const bonuses = {
    // 10 ج.م لكل عضو جديد
    newMembersBonus: round.newMembers * 10,

    // 2% إضافي إذا تجاوز 10,000 ج.م
    highVolumeBonus: round.stats.totalAmount > 10000
      ? round.stats.totalAmount * 0.02
      : 0,

    // 50 ج.م للتوزيع المثالي (100% استلام)
    perfectDistribution: round.stats.collectionRate === 1
      ? 50
      : 0,

    // 30 ج.م للالتزام بالوقت
    onTimeBonus: round.distributionDelay <= 0
      ? 30
      : 0,

    // 20 ج.م للتقييم الممتاز
    ratingBonus: round.avgRating >= 4.9
      ? 20
      : 0
  };

  // 3. الخصومات
  const deductions = {
    // 5 ج.م لكل ساعة تأخير
    lateDistribution: Math.max(0, round.distributionDelay) * 5,

    // 10 ج.م لكل شكوى مؤكدة
    complaints: round.confirmedComplaints * 10,

    // 2% من قيمة المرتجعات
    refunds: round.refundsAmount * 0.02
  };

  const total = baseAmount
    + Object.values(bonuses).reduce((a, b) => a + b, 0)
    - Object.values(deductions).reduce((a, b) => a + b, 0);

  return {
    leaderId: leader.id,
    roundId: round.id,
    baseCommission: { roundTotal: round.stats.totalAmount, commissionRate: baseRate, baseAmount },
    bonuses,
    deductions,
    total: Math.max(0, total), // لا يمكن أن تكون سالبة
    status: 'pending'
  };
}
```

---

## 3. قاعدة البيانات (Database Schema)

```sql
-- =====================================================
-- Community Buying Groups Database Schema
-- =====================================================

-- 1. المجموعات المجتمعية
CREATE TABLE community_groups (
    id INT AUTO_INCREMENT PRIMARY KEY,
    group_code VARCHAR(20) UNIQUE NOT NULL,

    -- معلومات المجموعة
    name VARCHAR(100) NOT NULL,
    name_ar VARCHAR(100) NOT NULL,
    description TEXT,
    description_ar TEXT,
    group_type ENUM('residential', 'office', 'university', 'club', 'neighborhood', 'other') NOT NULL,

    -- الموقع
    governorate VARCHAR(50) NOT NULL,
    city VARCHAR(100) NOT NULL,
    district VARCHAR(100),
    area VARCHAR(100) NOT NULL,
    address TEXT,
    address_ar TEXT,
    lat DECIMAL(10, 8),
    lng DECIMAL(11, 8),
    coverage_radius_km DECIMAL(4, 2) DEFAULT 1.0,

    -- القائد
    leader_id INT NOT NULL,
    backup_leader_id INT,

    -- إعدادات العمولة
    leader_commission_rate DECIMAL(4, 2) DEFAULT 5.00,
    platform_commission_rate DECIMAL(4, 2) DEFAULT 2.00,

    -- إعدادات الطلبات
    min_order_amount DECIMAL(10, 2) DEFAULT 50,
    max_order_amount DECIMAL(10, 2) DEFAULT 5000,
    order_round_frequency ENUM('daily', 'twice_weekly', 'weekly', 'biweekly') DEFAULT 'weekly',

    -- جدول الطلبات
    order_days JSON,            -- ["sunday", "wednesday"]
    order_deadline_time TIME,   -- "20:00"
    distribution_day VARCHAR(10),
    distribution_time_from TIME,
    distribution_time_to TIME,

    -- الحدود
    max_members INT DEFAULT 100,
    members_count INT DEFAULT 0,

    -- الإحصائيات
    total_rounds INT DEFAULT 0,
    total_orders INT DEFAULT 0,
    total_revenue DECIMAL(12, 2) DEFAULT 0,
    avg_order_value DECIMAL(10, 2) DEFAULT 0,
    avg_rating DECIMAL(3, 2) DEFAULT 5.00,

    -- الحالة
    status ENUM('pending_approval', 'active', 'paused', 'closed') DEFAULT 'pending_approval',
    approved_at DATETIME,
    approved_by INT,
    paused_reason TEXT,
    closed_reason TEXT,

    -- الصور
    group_photo VARCHAR(500),
    cover_photo VARCHAR(500),

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (leader_id) REFERENCES users(id),
    FOREIGN KEY (backup_leader_id) REFERENCES users(id),
    INDEX idx_status (status),
    INDEX idx_location (governorate, city, district),
    INDEX idx_leader (leader_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 2. القادة
CREATE TABLE community_leaders (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT UNIQUE NOT NULL,

    -- المعلومات الشخصية
    full_name VARCHAR(100) NOT NULL,
    full_name_ar VARCHAR(100),
    phone VARCHAR(20) NOT NULL,
    email VARCHAR(255),
    profile_photo VARCHAR(500),

    -- الهوية
    national_id VARCHAR(20) NOT NULL,
    national_id_verified BOOLEAN DEFAULT FALSE,
    national_id_image_front VARCHAR(500),
    national_id_image_back VARCHAR(500),

    -- التحقق
    kyc_status ENUM('pending', 'verified', 'rejected') DEFAULT 'pending',
    kyc_verified_at DATETIME,
    kyc_rejection_reason TEXT,
    background_check_status ENUM('pending', 'passed', 'failed', 'not_required') DEFAULT 'not_required',

    -- العنوان
    residence_address TEXT,
    residence_verified BOOLEAN DEFAULT FALSE,
    residency_months INT,

    -- الموارد
    has_storage_space BOOLEAN DEFAULT FALSE,
    storage_capacity_sqm DECIMAL(6, 2),
    has_vehicle BOOLEAN DEFAULT FALSE,
    vehicle_type VARCHAR(50),

    -- التوفر
    availability_hours JSON,   -- {"sunday": ["09:00-12:00", "16:00-20:00"], ...}
    preferred_distribution_days JSON,

    -- التدريب
    training_completed BOOLEAN DEFAULT FALSE,
    training_completed_at DATETIME,
    training_score INT,

    -- المستوى والشارات
    leader_level ENUM('beginner', 'regular', 'advanced', 'expert', 'master') DEFAULT 'beginner',
    badges JSON,

    -- الإحصائيات
    total_groups INT DEFAULT 0,
    active_groups INT DEFAULT 0,
    total_members INT DEFAULT 0,
    total_rounds INT DEFAULT 0,
    total_orders_value DECIMAL(12, 2) DEFAULT 0,
    total_earnings DECIMAL(12, 2) DEFAULT 0,
    avg_rating DECIMAL(3, 2) DEFAULT 5.00,
    total_ratings INT DEFAULT 0,

    -- المالية
    wallet_balance DECIMAL(10, 2) DEFAULT 0,
    pending_earnings DECIMAL(10, 2) DEFAULT 0,
    payout_method ENUM('bank_transfer', 'vodafone_cash', 'instapay'),
    bank_name VARCHAR(100),
    bank_account_number VARCHAR(50),
    mobile_wallet_number VARCHAR(20),

    -- الحالة
    status ENUM('pending', 'trial', 'active', 'suspended', 'banned') DEFAULT 'pending',
    trial_start_date DATE,
    trial_end_date DATE,
    suspension_reason TEXT,
    suspension_until DATETIME,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (user_id) REFERENCES users(id),
    INDEX idx_status (status),
    INDEX idx_level (leader_level)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 3. أعضاء المجموعات
CREATE TABLE community_group_members (
    id INT AUTO_INCREMENT PRIMARY KEY,
    group_id INT NOT NULL,
    user_id INT NOT NULL,

    -- معلومات العضو
    member_name VARCHAR(100) NOT NULL,
    member_phone VARCHAR(20) NOT NULL,
    member_email VARCHAR(255),

    -- معلومات السكن
    building_name VARCHAR(100),
    building_number VARCHAR(20),
    floor_number VARCHAR(10),
    apartment_number VARCHAR(20),
    landmark TEXT,

    -- التفضيلات
    preferred_collection_time ENUM('morning', 'afternoon', 'evening') DEFAULT 'evening',
    can_collect_for_neighbors BOOLEAN DEFAULT FALSE,
    preferred_payment_method ENUM('cash', 'wallet', 'card') DEFAULT 'cash',
    communication_preference ENUM('whatsapp', 'sms', 'app', 'call') DEFAULT 'whatsapp',

    -- المستوى
    membership_level ENUM('new', 'regular', 'trusted', 'vip') DEFAULT 'new',
    level_upgraded_at DATETIME,

    -- الإحصائيات
    total_orders INT DEFAULT 0,
    total_spent DECIMAL(10, 2) DEFAULT 0,
    avg_order_value DECIMAL(10, 2) DEFAULT 0,
    no_show_count INT DEFAULT 0,
    on_time_collection_rate DECIMAL(5, 2) DEFAULT 100,

    -- الحالة
    status ENUM('pending', 'active', 'suspended', 'removed') DEFAULT 'pending',
    joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    approved_at DATETIME,
    approved_by INT,
    removed_at DATETIME,
    removal_reason TEXT,

    FOREIGN KEY (group_id) REFERENCES community_groups(id),
    FOREIGN KEY (user_id) REFERENCES users(id),
    UNIQUE KEY unique_membership (group_id, user_id),
    INDEX idx_group (group_id),
    INDEX idx_user (user_id),
    INDEX idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 4. جولات الطلب
CREATE TABLE community_order_rounds (
    id INT AUTO_INCREMENT PRIMARY KEY,
    round_code VARCHAR(20) UNIQUE NOT NULL,
    group_id INT NOT NULL,

    -- المعلومات
    title VARCHAR(100),
    title_ar VARCHAR(100),
    description TEXT,

    -- التوقيت
    order_start DATETIME NOT NULL,
    order_deadline DATETIME NOT NULL,
    distribution_date DATE NOT NULL,
    distribution_time_from TIME NOT NULL,
    distribution_time_to TIME NOT NULL,

    -- الإعدادات
    min_order_amount DECIMAL(10, 2),
    max_orders INT,
    allow_late_orders BOOLEAN DEFAULT FALSE,
    late_order_fee DECIMAL(10, 2) DEFAULT 10,

    -- الإحصائيات
    orders_count INT DEFAULT 0,
    total_amount DECIMAL(10, 2) DEFAULT 0,
    unique_members INT DEFAULT 0,
    avg_order_value DECIMAL(10, 2) DEFAULT 0,
    items_count INT DEFAULT 0,

    -- التوزيع
    distribution_location_type ENUM('leader_home', 'building_entrance', 'meeting_point', 'custom'),
    distribution_address TEXT,
    distribution_lat DECIMAL(10, 8),
    distribution_lng DECIMAL(11, 8),
    distribution_instructions TEXT,

    -- إحصائيات التوزيع
    collected_orders INT DEFAULT 0,
    pending_orders INT DEFAULT 0,
    no_show_orders INT DEFAULT 0,
    collected_amount DECIMAL(10, 2) DEFAULT 0,
    pending_amount DECIMAL(10, 2) DEFAULT 0,

    -- العمولات
    leader_commission DECIMAL(10, 2) DEFAULT 0,
    platform_commission DECIMAL(10, 2) DEFAULT 0,

    -- الحالة
    status ENUM('scheduled', 'open', 'closed', 'processing', 'shipped', 'ready', 'distributing', 'completed', 'cancelled') DEFAULT 'scheduled',

    -- التوقيتات الفعلية
    actual_distribution_start DATETIME,
    actual_distribution_end DATETIME,
    distribution_delay_minutes INT DEFAULT 0,

    -- التقييم
    avg_rating DECIMAL(3, 2),
    total_ratings INT DEFAULT 0,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (group_id) REFERENCES community_groups(id),
    INDEX idx_group (group_id),
    INDEX idx_status (status),
    INDEX idx_deadline (order_deadline)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 5. طلبات الأعضاء
CREATE TABLE community_member_orders (
    id INT AUTO_INCREMENT PRIMARY KEY,
    order_code VARCHAR(20) UNIQUE NOT NULL,
    round_id INT NOT NULL,
    member_id INT NOT NULL,
    user_id INT NOT NULL,

    -- المنتجات
    items JSON NOT NULL,
    items_count INT NOT NULL,

    -- المبالغ
    subtotal DECIMAL(10, 2) NOT NULL,
    discount DECIMAL(10, 2) DEFAULT 0,
    late_fee DECIMAL(10, 2) DEFAULT 0,
    total DECIMAL(10, 2) NOT NULL,

    -- الدفع
    payment_method ENUM('cash', 'wallet', 'card', 'credit') DEFAULT 'cash',
    payment_status ENUM('pending', 'paid', 'refunded', 'failed') DEFAULT 'pending',
    paid_at DATETIME,
    payment_reference VARCHAR(100),

    -- الاستلام
    collection_status ENUM('pending', 'notified', 'ready', 'collected', 'delegate', 'no_show', 'rescheduled') DEFAULT 'pending',
    delegate_name VARCHAR(100),
    delegate_phone VARCHAR(20),
    collected_at DATETIME,
    collected_by VARCHAR(100),

    -- ملاحظات
    member_notes TEXT,
    leader_notes TEXT,

    -- التقييم
    rating INT,
    review TEXT,
    rated_at DATETIME,

    is_late_order BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (round_id) REFERENCES community_order_rounds(id),
    FOREIGN KEY (member_id) REFERENCES community_group_members(id),
    FOREIGN KEY (user_id) REFERENCES users(id),
    INDEX idx_round (round_id),
    INDEX idx_member (member_id),
    INDEX idx_status (collection_status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 6. عمولات القادة
CREATE TABLE community_leader_earnings (
    id INT AUTO_INCREMENT PRIMARY KEY,
    leader_id INT NOT NULL,
    group_id INT NOT NULL,
    round_id INT NOT NULL,

    -- العمولة الأساسية
    round_total DECIMAL(10, 2) NOT NULL,
    commission_rate DECIMAL(4, 2) NOT NULL,
    base_commission DECIMAL(10, 2) NOT NULL,

    -- المكافآت
    new_members_bonus DECIMAL(10, 2) DEFAULT 0,
    high_volume_bonus DECIMAL(10, 2) DEFAULT 0,
    perfect_distribution_bonus DECIMAL(10, 2) DEFAULT 0,
    on_time_bonus DECIMAL(10, 2) DEFAULT 0,
    rating_bonus DECIMAL(10, 2) DEFAULT 0,

    -- الخصومات
    late_deduction DECIMAL(10, 2) DEFAULT 0,
    complaints_deduction DECIMAL(10, 2) DEFAULT 0,
    refunds_deduction DECIMAL(10, 2) DEFAULT 0,

    -- الإجمالي
    total_bonuses DECIMAL(10, 2) DEFAULT 0,
    total_deductions DECIMAL(10, 2) DEFAULT 0,
    net_amount DECIMAL(10, 2) NOT NULL,

    -- الحالة
    status ENUM('pending', 'approved', 'paid', 'disputed') DEFAULT 'pending',
    approved_at DATETIME,
    approved_by INT,
    paid_at DATETIME,
    payout_reference VARCHAR(100),

    -- النزاع
    dispute_reason TEXT,
    dispute_resolved_at DATETIME,
    dispute_resolution TEXT,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (leader_id) REFERENCES community_leaders(id),
    FOREIGN KEY (group_id) REFERENCES community_groups(id),
    FOREIGN KEY (round_id) REFERENCES community_order_rounds(id),
    INDEX idx_leader (leader_id),
    INDEX idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```

---

## 4. واجهات برمجة التطبيقات (API Specification)

### 4.1 Leaders API

```typescript
// تسجيل كقائد
POST   /api/community/leaders/register
GET    /api/community/leaders/me
PUT    /api/community/leaders/me
POST   /api/community/leaders/me/documents
GET    /api/community/leaders/me/training

// إدارة المجموعات
POST   /api/community/groups/create
GET    /api/community/groups/:id
PUT    /api/community/groups/:id
GET    /api/community/groups/:id/members
POST   /api/community/groups/:id/members/:memberId/approve
DELETE /api/community/groups/:id/members/:memberId

// جولات الطلب
POST   /api/community/groups/:id/rounds/create
GET    /api/community/groups/:id/rounds
GET    /api/community/rounds/:id
PUT    /api/community/rounds/:id/status
GET    /api/community/rounds/:id/orders

// التوزيع
POST   /api/community/rounds/:id/distribution/start
POST   /api/community/rounds/:id/orders/:orderId/collect
POST   /api/community/rounds/:id/distribution/complete

// الأرباح
GET    /api/community/leaders/me/earnings
GET    /api/community/leaders/me/earnings/summary
POST   /api/community/leaders/me/payout
```

### 4.2 Members API

```typescript
// البحث والانضمام
GET    /api/community/groups/nearby
GET    /api/community/groups/:id
POST   /api/community/groups/:id/join
GET    /api/community/groups/:id/catalog

// الطلبات
GET    /api/community/groups/:id/current-round
POST   /api/community/rounds/:id/orders
GET    /api/community/orders/:id
PUT    /api/community/orders/:id
DELETE /api/community/orders/:id

// الاستلام
GET    /api/community/orders/:id/collection-status
POST   /api/community/orders/:id/delegate
POST   /api/community/orders/:id/rate

// العضوية
GET    /api/community/memberships
GET    /api/community/memberships/:id/stats
```

---

## 5. خطة الإطلاق

### المرحلة 1: التجريبية (4 أسابيع)
```
- 5 مجموعات في 3 أحياء بالقاهرة
- 10-20 عضو لكل مجموعة
- جولة واحدة أسبوعياً
- التركيز على المنتجات الأساسية
```

### المرحلة 2: التوسع المحلي (8 أسابيع)
```
- 50 مجموعة في القاهرة الكبرى
- 20-50 عضو لكل مجموعة
- جولتين أسبوعياً
- توسيع الكتالوج
```

### المرحلة 3: التوسع الوطني (12 أسبوع)
```
- 200+ مجموعة في المحافظات الرئيسية
- نظام تدريب القادة المتقدم
- كتالوج كامل
- تكامل مع التجار المحليين
```

---

## 6. معايير النجاح

| المقياس | الهدف (شهر 3) | الهدف (شهر 6) |
|---------|---------------|---------------|
| مجموعات نشطة | 50 | 200 |
| قادة معتمدين | 60 | 250 |
| أعضاء نشطين | 2,000 | 10,000 |
| GMV شهري | 500K ج.م | 3M ج.م |
| معدل الاستلام | 95% | 98% |
| رضا الأعضاء | 4.3+ | 4.6+ |
| دخل القائد/شهر | 1,500 ج.م | 3,000 ج.م |

---

## 7. تحليل المخاطر (Risk Assessment)

| المخاطر | الاحتمال | التأثير | خطة التخفيف |
|---------|----------|---------|-------------|
| انسحاب القائد فجأة | متوسط | عالي | قائد احتياطي لكل مجموعة، نقل سلس للقيادة، فترة إشعار إلزامية |
| سوء توزيع الطلبات | منخفض | عالي | نظام تتبع، صور التسليم، تأكيد الاستلام |
| تأخر الدفع من الأعضاء | متوسط | متوسط | دفع مقدم إلزامي، نظام محفظة، خصم تلقائي |
| خلافات بين الأعضاء | منخفض | منخفض | قواعد واضحة، نظام شكاوى، وساطة |
| احتيال القائد | منخفض | عالي | KYC صارم، ضمان مالي، مراقبة المعاملات |
| عدم وصول الطلبات للحد الأدنى | متوسط | متوسط | تجميع مع مجموعات مجاورة، عروض بديلة |

---

## 8. خطة الطوارئ (Contingency Plan)

### 8.1 ماذا لو انسحب القائد؟
```
السيناريو: القائد يريد الانسحاب أو أصبح غير متاح
الإجراءات:
1. إلزام القائد بإشعار مسبق 14 يوم
2. البحث عن قائد بديل من الأعضاء المميزين
3. إذا لم يوجد: دمج المجموعة مع مجموعة مجاورة
4. تسوية جميع المستحقات قبل الانتقال
5. إشعار جميع الأعضاء وضمان استمرار الخدمة
```

### 8.2 ماذا لو رفض أعضاء استلام طلباتهم؟
```
السيناريو: الطلبات وصلت للقائد لكن أعضاء لم يستلموا
الإجراءات:
1. إشعارات متعددة (SMS, WhatsApp, Call)
2. مهلة 72 ساعة للاستلام
3. بعد المهلة: رسوم تخزين 5 ج.م/يوم
4. بعد 7 أيام: إرجاع للمخزن + خصم من المحفظة
5. تسجيل في "قائمة الموثوقية المنخفضة"
```

### 8.3 ماذا لو لم تصل الشحنة؟
```
السيناريو: الطلب المجمع تأخر أو لم يصل
الإجراءات:
1. تتبع فوري مع شركة الشحن
2. إبلاغ القائد وجميع الأعضاء
3. إذا ضاع: تعويض كامل + خصم 20% على الطلب القادم
4. تحقيق مع شركة الشحن ومطالبة بالتعويض
```

---

## 9. مؤشرات أداء إضافية (Additional KPIs)

| المقياس | التعريف | الهدف | طريقة الحساب |
|---------|---------|-------|--------------|
| **تكلفة اكتساب قائد (LAC)** | تكلفة جلب قائد جديد | < 200 ج.م | تكاليف التوظيف والتدريب ÷ قادة جدد |
| **معدل نمو المجموعات** | أعضاء جدد شهرياً | > 20% | أعضاء جدد ÷ إجمالي الأعضاء |
| **معدل نشاط الأعضاء** | أعضاء يطلبون شهرياً | > 70% | أعضاء طلبوا ÷ إجمالي الأعضاء |
| **GMV لكل قائد** | حجم مبيعات القائد | > 15,000 ج.م/شهر | إجمالي GMV ÷ قادة نشطين |
| **معدل الاستلام في الوقت** | استلام خلال 48 ساعة | > 90% | استلام في الوقت ÷ إجمالي الطلبات |
| **LTV:LAC Ratio** | كفاءة استثمار القادة | > 20:1 | إيرادات من القائد ÷ تكلفة اكتسابه |

---

**نهاية المستند**
