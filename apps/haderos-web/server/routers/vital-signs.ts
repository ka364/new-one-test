...
class SimpleFungalSimulator {
  ...
  private simulateResourceDistribution(
    intensity: number,
  ...
  private simulateStressResponse(
    intensity: number,
    events: SimulationResult['events'],
    step: number
  ) {
  ...
  private simulateLearningPropagation(
    intensity: number,
    events: SimulationResult['events'],
    step: number
  ) {
  ...
  private calculateMetrics(
    ...
    return {
      resourceEfficiency: Math.min(
        100,
        (endState.totalResources / startState.totalResources) * 100
      ),
...
async function calculateProtocolMetrics(protocolId: string): Promise<{
  ...
  try {
    ...
    const baseMetrics: Record<
      string,
      { efficiency: number; reliability: number; adaptability: number }
    > = {
