...
export default function CashFlowForecast() {
  ...
  const displayData =
    period === '3months' ? data.slice(0, 3) : period === '6months' ? data.slice(0, 6) : data;
    ...
    <div className="space-y-6">
      ...
      {deficitMonths > 0 && (
        <Alert className="bg-red-50 border-red-200">
          ...
          <AlertDescription className="text-red-800">
            ⚠️ تحذير: يوجد {deficitMonths} شهر{deficitMonths > 1 ? '' : ''} سيشهد عجز نقدي حسب هذا
            السيناريو
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${avgMonthlyNetFlow >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              ...
              <tbody>
                {displayData.map((month, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td
                      className={`px-4 py-3 font-semibold ${month.netCashFlow >= 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {(month.netCashFlow / 1000).toFixed(0)}K
                    </td>
                    <td
                      className={`px-4 py-3 font-medium ${month.cumulativeBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`}
                    >
                      {(month.cumulativeBalance / 1000000).toFixed(2)}M
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${month.isDeficit ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}
                      >
        ...
        <CardContent className="text-blue-800 space-y-2">
          <ul className="space-y-2">
            ...
            <li className="flex gap-2">
              ...
              <span>
                {avgMonthlyNetFlow > 0
                  ? `متوسط التدفق الشهري موجب: ${(avgMonthlyNetFlow / 1000).toFixed(0)}K`
                  : 'يجب مراجعة نموذج الإيرادات والمصروفات'}
              </span>
            </li>
            <li className="flex gap-2">
              <span>✓</span>
