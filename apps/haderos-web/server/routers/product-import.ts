...
import { migrateProductImages, type ImageMigrationResult } from '../integrations/image-migration';
        ...
        for (const product of validProducts) {
          try {
            ...
            if (input.migrateImages && product.images.length > 0) {
              const migration = await migrateProductImages(product.images, product.modelCode);
