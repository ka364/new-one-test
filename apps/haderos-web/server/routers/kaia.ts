...
const SHARIA_RULES: ComplianceRule[] = [
  ...
  },
  {
    ...
    check: (tx) => {
      ...
      const isLoan =
        tx.transactionType === 'loan' || tx.description?.toLowerCase().includes('loan');
  ...
  // Gharar (Uncertainty) Detection
  {
    ...
    check: (tx) => {
      ...
      const isSpeculative =
        tx.description?.toLowerCase().includes('speculation') ||
  ...
  },
  {
    ...
    check: (tx) => {
      ...
      const sellingNonExistent =
        tx.goodsStatus === 'not_yet_produced' || tx.goodsStatus === 'not_owned';
  ...
  // Haram Goods Detection
  {
    ...
    check: (tx) => {
      ...
      return haramKeywords.some(
        (keyword) => description.includes(keyword) || goods.includes(keyword)
      );
    },
