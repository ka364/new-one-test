# Product Requirements Document (PRD)
# نظام التوصيل بالجمهور - Crowdsourced Delivery System

---

## 📋 معلومات المستند

| البند | القيمة |
|-------|--------|
| **رقم المستند** | PRD-HADEROS-CD-002 |
| **الإصدار** | 1.0 |
| **تاريخ الإنشاء** | 2 يناير 2026 |
| **الحالة** | مسودة للمراجعة |
| **المؤلف** | فريق HADEROS |

---

## 1. نظرة عامة (Overview)

### 1.1 ملخص تنفيذي
نظام التوصيل بالجمهور (Crowdsourced Delivery) يتيح لأي شخص يمتلك وسيلة نقل أن يصبح مندوب توصيل مستقل. يهدف النظام لحل مشكلة "الميل الأخير" (Last-Mile Delivery) في مصر من خلال شبكة موزعة من السائقين المستقلين.

### 1.2 الرؤية
"توصيل أسرع وأرخص لكل مصري، وفرصة دخل لكل من يملك وسيلة نقل"

### 1.3 المشكلات التي نحلها

| المشكلة | الحل |
|---------|------|
| ارتفاع تكلفة الشحن | تكلفة أقل 30-40% من الشركات التقليدية |
| بطء التوصيل (2-5 أيام) | توصيل نفس اليوم أو خلال ساعات |
| عدم تغطية بعض المناطق | سائقين محليين يعرفون كل منطقة |
| بطالة الشباب | فرص عمل مرنة |
| ذروة الطلب | مرونة في زيادة السائقين |

### 1.4 الجمهور المستهدف

**السائقون (Drivers):**
| الشريحة | الوصف | الحافز |
|---------|-------|--------|
| طلاب الجامعات | وقت فراغ، يحتاجون دخل | مرونة + دخل إضافي |
| سائقو التاكسي/أوبر | لديهم سيارات، أوقات فراغ | دخل إضافي |
| عمال التوصيل | خبرة في التوصيل | دخل أفضل |
| أصحاب الموتوسيكلات | شباب، سرعة | دخل ثابت |
| ربات البيوت | لديهن سيارات | دخل إضافي بدوام جزئي |

**العملاء:**
| الشريحة | الاحتياج |
|---------|----------|
| متاجر E-commerce | توصيل سريع للعملاء |
| مطاعم | توصيل الطعام |
| صيدليات | توصيل الأدوية |
| أفراد | إرسال طرود شخصية |

---

## 2. المتطلبات الوظيفية (Functional Requirements)

### 2.1 نظام تسجيل السائقين

#### FR-CD-001: تسجيل سائق جديد
**الوصف:** عملية تسجيل وتحقق من السائقين الجدد

**خطوات التسجيل:**
```
1. المعلومات الأساسية
   ├── الاسم الكامل
   ├── رقم الهاتف (OTP)
   ├── البريد الإلكتروني
   └── تاريخ الميلاد

2. الهوية والتحقق
   ├── الرقم القومي
   ├── صورة البطاقة (وجه + ظهر)
   ├── صورة شخصية (Selfie مع البطاقة)
   └── التحقق من مطابقة الوجه (AI)

3. معلومات المركبة
   ├── نوع المركبة (دراجة/موتوسيكل/سيارة/فان)
   ├── الماركة والموديل
   ├── سنة الصنع
   ├── رقم اللوحات
   ├── صورة المركبة
   └── رخصة القيادة (إن وجدت)

4. المنطقة والتفضيلات
   ├── المناطق المفضلة للعمل
   ├── أوقات العمل المفضلة
   └── نوع الشحنات المفضلة

5. المعلومات المالية
   ├── رقم الحساب البنكي / المحفظة
   └── طريقة استلام الأرباح
```

**التحقق من الهوية (KYC):**
```typescript
interface DriverKYC {
  // المستوى 1: أساسي
  level1: {
    phoneVerified: boolean;      // OTP
    emailVerified: boolean;
    nationalIdProvided: boolean;
  };

  // المستوى 2: متوسط
  level2: {
    nationalIdVerified: boolean;  // OCR + Manual review
    selfieMatched: boolean;       // Face matching AI
    addressVerified: boolean;     // Optional
  };

  // المستوى 3: كامل
  level3: {
    backgroundCheck: boolean;     // Criminal record check
    vehicleVerified: boolean;     // Vehicle ownership
    licenseVerified: boolean;     // If applicable
  };
}

// مستويات العمل حسب التحقق
const workLevels = {
  level1: {
    canDeliverDocuments: true,
    canDeliverPackages: false,
    maxValuePerDelivery: 500,
    dailyLimit: 10
  },
  level2: {
    canDeliverDocuments: true,
    canDeliverPackages: true,
    maxValuePerDelivery: 5000,
    dailyLimit: 30
  },
  level3: {
    canDeliverDocuments: true,
    canDeliverPackages: true,
    canDeliverHighValue: true,
    maxValuePerDelivery: 50000,
    dailyLimit: 100
  }
};
```

---

#### FR-CD-002: إدارة توفر السائق
**الوصف:** نظام لإدارة حالة توفر السائق

**حالات السائق:**
```typescript
enum DriverStatus {
  OFFLINE = 'offline',           // غير متصل
  ONLINE_AVAILABLE = 'online_available',  // متصل ومتاح
  ONLINE_BUSY = 'online_busy',   // متصل ومشغول (في توصيلة)
  ON_BREAK = 'on_break',         // في استراحة
  SUSPENDED = 'suspended'        // موقوف
}
```

**تحديث الموقع:**
```typescript
interface LocationUpdate {
  driverId: number;
  lat: number;
  lng: number;
  accuracy: number;        // بالمتر
  speed: number;           // km/h
  heading: number;         // 0-360 degrees
  battery: number;         // نسبة البطارية
  timestamp: Date;
  source: 'gps' | 'network' | 'fused';
}

// إعدادات تحديث الموقع
const locationSettings = {
  updateInterval: {
    idle: 60000,           // كل دقيقة عندما لا يوجد طلب
    assigned: 15000,       // كل 15 ثانية عند وجود طلب
    inTransit: 5000        // كل 5 ثواني أثناء التوصيل
  },
  accuracyThreshold: 100,  // متر
  batteryThreshold: 15     // تحذير عند 15%
};
```

---

### 2.2 نظام طلبات التوصيل

#### FR-CD-003: إنشاء طلب توصيل
**الوصف:** إنشاء طلب توصيل جديد

**المدخلات:**
```typescript
interface CreateDeliveryRequest {
  // مصدر الطلب
  source: 'order' | 'merchant' | 'individual';
  sourceId?: number;         // رقم الطلب إن وجد

  // معلومات الاستلام
  pickup: {
    address: string;
    addressAr: string;
    lat: number;
    lng: number;
    contactName: string;
    contactPhone: string;
    notes?: string;
    timeWindow?: {
      from: Date;
      to: Date;
    };
  };

  // معلومات التسليم
  delivery: {
    address: string;
    addressAr: string;
    lat: number;
    lng: number;
    governorate: string;
    city: string;
    contactName: string;
    contactPhone: string;
    notes?: string;
    timeWindow?: {
      from: Date;
      to: Date;
    };
  };

  // معلومات الشحنة
  package: {
    type: 'document' | 'small_package' | 'medium_package' | 'large_package' | 'fragile' | 'food';
    weight?: number;         // kg
    dimensions?: {
      length: number;
      width: number;
      height: number;
    };
    description: string;
    value?: number;          // للتأمين
    requiresSignature: boolean;
    isFragile: boolean;
    temperatureControlled: boolean;
  };

  // خيارات التوصيل
  options: {
    isExpress: boolean;      // توصيل سريع
    scheduledTime?: Date;    // توصيل مجدول
    codAmount?: number;      // الدفع عند الاستلام
    tip?: number;            // بقشيش للسائق
  };
}
```

**حساب التكلفة:**
```typescript
async function calculateDeliveryFee(request: CreateDeliveryRequest): Promise<FeeCalculation> {
  // 1. حساب المسافة
  const distance = await calculateDistance(
    request.pickup.lat, request.pickup.lng,
    request.delivery.lat, request.delivery.lng
  );

  // 2. الحصول على منطقة التسعير
  const zone = await getDeliveryZone(request.delivery.governorate);

  // 3. حساب الرسوم الأساسية
  let baseFee = zone.basePrice;
  let distanceFee = distance * zone.pricePerKm;

  // 4. رسوم إضافية
  let additionalFees = 0;

  // نوع الشحنة
  const packageMultiplier = {
    document: 0.8,
    small_package: 1.0,
    medium_package: 1.3,
    large_package: 1.6,
    fragile: 1.5,
    food: 1.2
  };
  additionalFees += baseFee * (packageMultiplier[request.package.type] - 1);

  // التوصيل السريع
  if (request.options.isExpress) {
    additionalFees += (baseFee + distanceFee) * (zone.expressMultiplier - 1);
  }

  // ساعات الذروة
  if (isPeakHour(zone)) {
    additionalFees += (baseFee + distanceFee) * (zone.peakHourMultiplier - 1);
  }

  // الوزن الزائد
  if (request.package.weight && request.package.weight > 5) {
    additionalFees += (request.package.weight - 5) * 5; // 5 ج.م لكل كجم زائد
  }

  // التأمين
  let insuranceFee = 0;
  if (request.package.value) {
    insuranceFee = request.package.value * 0.01; // 1% من القيمة
  }

  // الإجمالي
  const subtotal = baseFee + distanceFee + additionalFees + insuranceFee;
  const platformFee = subtotal * 0.15;  // 15% عمولة المنصة
  const driverPayout = subtotal - platformFee;

  return {
    breakdown: {
      baseFee,
      distanceFee,
      additionalFees,
      insuranceFee,
      platformFee
    },
    subtotal,
    total: subtotal + (request.options.codAmount ? 0 : 0), // COD لا يؤثر على رسوم التوصيل
    driverPayout,
    estimatedDuration: calculateDuration(distance),
    distance
  };
}
```

---

#### FR-CD-004: نظام المطابقة الذكي (Smart Matching)
**الوصف:** خوارزمية لاختيار أفضل سائق للطلب

```typescript
interface MatchingCriteria {
  weights: {
    distance: number;        // 30%
    rating: number;          // 25%
    successRate: number;     // 20%
    vehicleMatch: number;    // 15%
    availability: number;    // 10%
  };
}

async function findBestDrivers(
  request: DeliveryRequest,
  limit: number = 5
): Promise<RankedDriver[]> {

  // 1. الحصول على السائقين المتاحين في النطاق
  const nearbyDrivers = await getNearbyAvailableDrivers(
    request.pickup.lat,
    request.pickup.lng,
    30 // km radius
  );

  // 2. تصفية حسب نوع المركبة
  const eligibleDrivers = nearbyDrivers.filter(driver =>
    isVehicleSuitable(driver.vehicleType, request.package.type)
  );

  // 3. حساب النقاط لكل سائق
  const scoredDrivers = eligibleDrivers.map(driver => {
    let score = 0;

    // أ. المسافة (0-30 نقطة)
    const distanceToPickup = calculateDistance(
      driver.currentLat, driver.currentLng,
      request.pickup.lat, request.pickup.lng
    );
    score += Math.max(0, 30 - (distanceToPickup * 2));

    // ب. التقييم (0-25 نقطة)
    score += (driver.averageRating / 5) * 25;

    // ج. معدل النجاح (0-20 نقطة)
    if (driver.totalDeliveries > 10) {
      const successRate = driver.successfulDeliveries / driver.totalDeliveries;
      score += successRate * 20;
    } else {
      score += 15; // نقاط افتراضية للسائقين الجدد
    }

    // د. مطابقة المركبة (0-15 نقطة)
    score += getVehicleMatchScore(driver.vehicleType, request.package.type);

    // هـ. التوفر والاستعداد (0-10 نقاط)
    const minutesSinceLastActivity = getMinutesSince(driver.lastLocationUpdate);
    if (minutesSinceLastActivity < 5) score += 10;
    else if (minutesSinceLastActivity < 15) score += 7;
    else if (minutesSinceLastActivity < 30) score += 4;

    // و. منطقة مفضلة (Bonus)
    if (driver.preferredAreas?.includes(request.delivery.governorate)) {
      score += 5;
    }

    return {
      driver,
      score,
      distanceToPickup,
      estimatedArrival: calculateETA(distanceToPickup)
    };
  });

  // 4. ترتيب وإرجاع الأفضل
  return scoredDrivers
    .sort((a, b) => b.score - a.score)
    .slice(0, limit);
}

// التحقق من مناسبة المركبة
function isVehicleSuitable(vehicleType: string, packageType: string): boolean {
  const suitability: Record<string, string[]> = {
    bicycle: ['document'],
    motorcycle: ['document', 'small_package', 'food'],
    car: ['document', 'small_package', 'medium_package', 'fragile', 'food'],
    van: ['document', 'small_package', 'medium_package', 'large_package', 'fragile'],
    truck: ['medium_package', 'large_package']
  };

  return suitability[vehicleType]?.includes(packageType) || false;
}
```

---

#### FR-CD-005: تدفق الطلب (Order Flow)
**الوصف:** مراحل الطلب من الإنشاء حتى التسليم

```
                    ┌──────────────────────────────────────────────────────────────┐
                    │                        تدفق طلب التوصيل                      │
                    └──────────────────────────────────────────────────────────────┘

     ┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
     │ pending │────►│ assigned│────►│ heading │────►│ picked  │────►│in_transit│
     │         │     │         │     │ pickup  │     │   up    │     │         │
     └─────────┘     └─────────┘     └─────────┘     └─────────┘     └─────────┘
          │               │               │               │               │
          │               │               │               │               │
          ▼               ▼               ▼               ▼               ▼
     ┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
     │cancelled│     │cancelled│     │cancelled│     │cancelled│     │ arrived │
     │         │     │         │     │         │     │         │     │         │
     └─────────┘     └─────────┘     └─────────┘     └─────────┘     └─────────┘
                                                                           │
                                                                           │
                                                                           ▼
                                                                     ┌─────────┐
                                                                     │delivered│
                                                                     │         │
                                                                     └─────────┘
                                                                           │
                                                                           │
                                                                           ▼
                                                                     ┌─────────┐
                                                                     │completed│
                                                                     │ (rated) │
                                                                     └─────────┘
```

**تفاصيل كل مرحلة:**

```typescript
interface DeliveryStatusFlow {
  pending: {
    description: 'في انتظار سائق';
    timeout: 15 * 60 * 1000;  // 15 دقيقة
    onTimeout: 'escalate' | 'cancel';
    allowedActions: ['cancel', 'edit'];
  };

  assigned: {
    description: 'تم تعيين سائق';
    timeout: 10 * 60 * 1000;  // 10 دقائق
    onTimeout: 'reassign';
    allowedActions: ['cancel'];
    notifications: ['driver_assigned_customer', 'new_delivery_driver'];
  };

  heading_to_pickup: {
    description: 'السائق في الطريق للاستلام';
    trackingEnabled: true;
    allowedActions: ['cancel_with_fee'];
    notifications: ['driver_on_way_merchant'];
  };

  picked_up: {
    description: 'تم استلام الشحنة';
    requiresAction: 'confirm_pickup';
    requiresPhoto: true;
    allowedActions: [];
    notifications: ['package_picked_up_customer'];
  };

  in_transit: {
    description: 'جاري التوصيل';
    trackingEnabled: true;
    allowedActions: ['report_issue'];
    notifications: [];
  };

  arrived: {
    description: 'السائق وصل';
    timeout: 15 * 60 * 1000;  // 15 دقيقة للتسليم
    onTimeout: 'contact_support';
    allowedActions: ['confirm_delivery', 'report_issue'];
    notifications: ['driver_arrived_customer'];
  };

  delivered: {
    description: 'تم التسليم';
    requiresAction: 'delivery_confirmation';
    requiresPhoto: true;
    requiresSignature: (request) => request.package.requiresSignature;
    codCollection: (request) => request.options.codAmount > 0;
    notifications: ['delivery_completed_all'];
  };

  completed: {
    description: 'مكتمل ومقيّم';
    allowsRating: true;
    payoutProcessed: true;
  };

  cancelled: {
    description: 'ملغي';
    cancelledBy: 'customer' | 'driver' | 'system' | 'merchant';
    refundPolicy: (status, cancelledBy) => calculateRefund(status, cancelledBy);
  };

  failed: {
    description: 'فشل التوصيل';
    reasons: ['customer_unavailable', 'wrong_address', 'refused', 'other'];
    requiresProof: true;
    nextAction: 'return' | 'reschedule' | 'refund';
  };
}
```

---

#### FR-CD-006: التتبع المباشر (Live Tracking)
**الوصف:** نظام تتبع الشحنة في الوقت الفعلي

**للعميل:**
```typescript
interface CustomerTrackingView {
  // معلومات الطلب
  requestCode: string;
  status: DeliveryStatus;
  statusAr: string;

  // معلومات السائق (عند التعيين)
  driver?: {
    name: string;
    photo: string;
    phone: string;       // مخفي جزئياً: 010****5678
    vehicleType: string;
    vehiclePlate: string;
    rating: number;
  };

  // الموقع الحالي
  currentLocation?: {
    lat: number;
    lng: number;
    heading: number;
    lastUpdate: Date;
  };

  // التوقعات
  estimates: {
    arrivalTime: Date;
    remainingDistance: number;
    remainingDuration: number;
  };

  // المسار
  route?: {
    polyline: string;    // Encoded polyline
    waypoints: Array<{lat: number; lng: number}>;
  };

  // الأحداث
  timeline: Array<{
    status: string;
    statusAr: string;
    timestamp: Date;
    location?: {lat: number; lng: number};
    note?: string;
  }>;
}
```

**تحديث الموقع في الوقت الفعلي:**
```typescript
// WebSocket for real-time updates
class DeliveryTrackingSocket {
  private socket: WebSocket;

  connect(requestCode: string): void {
    this.socket = new WebSocket(`wss://api.haderos.com/tracking/${requestCode}`);

    this.socket.onmessage = (event) => {
      const update = JSON.parse(event.data);

      switch (update.type) {
        case 'location':
          this.onLocationUpdate(update.data);
          break;
        case 'status':
          this.onStatusChange(update.data);
          break;
        case 'eta':
          this.onETAUpdate(update.data);
          break;
      }
    };
  }

  private onLocationUpdate(data: LocationUpdate): void {
    // Update map marker
    // Update polyline
    // Recalculate ETA
  }
}
```

---

#### FR-CD-007: نظام الدفع للسائقين
**الوصف:** إدارة محفظة السائق والمدفوعات

```typescript
interface DriverWallet {
  driverId: number;
  balance: number;          // الرصيد الحالي
  pendingBalance: number;   // أرباح قيد المعالجة
  totalEarnings: number;    // إجمالي الأرباح
  totalPayouts: number;     // إجمالي المسحوبات
}

// أنواع المعاملات
enum TransactionType {
  DELIVERY_EARNING = 'delivery_earning',  // أجر توصيل
  TIP = 'tip',                            // بقشيش
  BONUS = 'bonus',                        // مكافأة
  PAYOUT = 'payout',                      // سحب
  PENALTY = 'penalty',                    // غرامة
  ADJUSTMENT = 'adjustment',              // تعديل
  COD_COLLECTED = 'cod_collected',        // COD محصل
  COD_REMITTED = 'cod_remitted'           // COD مسلم للتاجر
}

// حساب أرباح التوصيل
function calculateDriverEarning(delivery: CompletedDelivery): DriverEarning {
  const basePayout = delivery.fee.driverPayout;

  // البقشيش
  const tip = delivery.options.tip || 0;

  // المكافآت
  let bonus = 0;

  // مكافأة التوصيل السريع
  if (delivery.actualDuration < delivery.estimatedDuration * 0.8) {
    bonus += 10; // 10 ج.م للتوصيل أسرع من المتوقع
  }

  // مكافأة التقييم الممتاز
  if (delivery.customerRating === 5) {
    bonus += 5;
  }

  // مكافأة ساعات الذروة
  if (isPeakHour(delivery.deliveredAt)) {
    bonus += basePayout * 0.1; // 10% إضافي
  }

  // الخصومات
  let deductions = 0;

  // خصم التأخير
  if (delivery.actualDuration > delivery.estimatedDuration * 1.5) {
    deductions += 5;
  }

  return {
    basePayout,
    tip,
    bonus,
    deductions,
    total: basePayout + tip + bonus - deductions,
    breakdown: {
      deliveryFee: basePayout,
      expressBonus: bonus > 0 ? 10 : 0,
      ratingBonus: delivery.customerRating === 5 ? 5 : 0,
      peakBonus: isPeakHour(delivery.deliveredAt) ? basePayout * 0.1 : 0,
      tip,
      lateDeduction: delivery.actualDuration > delivery.estimatedDuration * 1.5 ? -5 : 0
    }
  };
}
```

**نظام السحب:**
```typescript
interface PayoutRequest {
  driverId: number;
  amount: number;
  method: 'bank_transfer' | 'vodafone_cash' | 'etisalat_cash' | 'orange_money' | 'instapay';
  accountDetails: {
    bankName?: string;
    accountNumber?: string;
    accountHolder?: string;
    phoneNumber?: string;  // للمحافظ
  };
}

// قواعد السحب
const payoutRules = {
  minAmount: 100,            // الحد الأدنى 100 ج.م
  maxAmountPerDay: 10000,    // الحد الأقصى 10,000 ج.م يومياً
  processingTime: {
    vodafone_cash: 'instant',
    etisalat_cash: 'instant',
    orange_money: 'instant',
    instapay: 'instant',
    bank_transfer: '1-2 business days'
  },
  fees: {
    vodafone_cash: 0,
    etisalat_cash: 0,
    orange_money: 0,
    instapay: 0,
    bank_transfer: amount => amount > 5000 ? 0 : 10
  }
};
```

---

#### FR-CD-008: نظام التقييم
**الوصف:** تقييم ثنائي الاتجاه بين العميل والسائق

```typescript
interface DeliveryRating {
  requestId: number;

  // تقييم العميل للسائق
  customerToDriver: {
    rating: 1 | 2 | 3 | 4 | 5;
    tags?: Array<
      | 'fast_delivery'        // توصيل سريع
      | 'professional'         // محترف
      | 'polite'               // مهذب
      | 'careful_handling'     // حريص على الشحنة
      | 'good_communication'   // تواصل جيد
      | 'late'                 // متأخر
      | 'rude'                 // غير مهذب
      | 'damaged_package'      // الشحنة تضررت
      | 'wrong_delivery'       // توصيل خاطئ
    >;
    comment?: string;
    tip?: number;
  };

  // تقييم السائق للعميل
  driverToCustomer?: {
    rating: 1 | 2 | 3 | 4 | 5;
    tags?: Array<
      | 'easy_location'        // موقع سهل
      | 'responsive'           // متجاوب
      | 'prepared'             // جاهز للاستلام
      | 'hard_to_find'         // صعب الوصول
      | 'unresponsive'         // لا يرد
      | 'delayed_pickup'       // تأخر في الاستلام
    >;
    comment?: string;
  };
}

// تأثير التقييم على السائق
function updateDriverRating(driverId: number, newRating: number): void {
  const driver = await getDriver(driverId);

  // حساب المتوسط المرجح (آخر 100 تقييم)
  const recentRatings = await getRecentRatings(driverId, 100);
  const newAverage = calculateWeightedAverage(recentRatings);

  // تحديث التقييم
  await updateDriver(driverId, { averageRating: newAverage });

  // التحقق من الإيقاف
  if (newAverage < 3.5 && driver.totalRatings > 20) {
    await flagDriverForReview(driverId, 'low_rating');
  }

  // مكافأة التقييم الممتاز
  if (newAverage >= 4.8 && driver.totalRatings > 50) {
    await awardBadge(driverId, 'top_rated');
  }
}
```

---

### 2.3 واجهات المستخدم

#### UI-CD-001: تطبيق السائق (Driver App)

**الشاشة الرئيسية:**
```
┌─────────────────────────────────────────────────────────────────┐
│  ☰                    HADEROS Driver                    [👤]    │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │     💰 اليوم: 450 ج.م     │     📦 8 توصيلات           │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │           ╭──────────────────────────────╮             │   │
│  │           │                              │             │   │
│  │           │       [    أنت متاح    ]     │             │   │
│  │           │           ● ONLINE           │             │   │
│  │           │                              │             │   │
│  │           ╰──────────────────────────────╯             │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ⚡ طلب جديد قريب منك!                                          │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  📍 المعادي ← 📍 مدينة نصر                              │   │
│  │  📏 12 كم  •  ⏱️ ~25 دقيقة  •  💰 65 ج.م                │   │
│  │  📦 طرد صغير                                            │   │
│  │                                                         │   │
│  │  ┌───────────────┐    ┌───────────────┐                │   │
│  │  │   ✓ قبول     │    │    ✗ رفض     │                │   │
│  │  └───────────────┘    └───────────────┘                │   │
│  │                                                         │   │
│  │  ⏰ متبقي 00:45 للرد                                    │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  📊 إحصائياتك                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  ⭐ 4.8    │    ✅ 98%    │    📦 156 هذا الشهر         │   │
│  │  تقييمك   │  نسبة النجاح │   توصيلاتك                  │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│   [🏠]        [📦]        [💰]        [📊]        [⚙️]         │
│   الرئيسية    الطلبات     المحفظة     الإحصائيات   الإعدادات    │
└─────────────────────────────────────────────────────────────────┘
```

**شاشة التوصيل النشط:**
```
┌─────────────────────────────────────────────────────────────────┐
│  ← رجوع           توصيل نشط #CD-2026-12345                      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │                      [خريطة GPS]                        │   │
│  │                                                         │   │
│  │           📍 ────────────────► 🏠                       │   │
│  │           أنت                  العميل                   │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  الحالة: جاري التوصيل 🚗                                │   │
│  │  ─────────────────────────────────────────────────────  │   │
│  │  ○ تم الاستلام ✓                                        │   │
│  │  ● جاري التوصيل ←                                       │   │
│  │  ○ تم التسليم                                           │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  📍 التوصيل إلى:                                                │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  أحمد محمد                                              │   │
│  │  15 شارع الملك فيصل، مدينة نصر                          │   │
│  │  الدور 5، شقة 12                                        │   │
│  │                                                         │   │
│  │  📞 الاتصال بالعميل    │    🗺️ فتح الخريطة             │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  📦 الشحنة:                                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  طرد صغير - إلكترونيات                                  │   │
│  │  💰 تحصيل عند الاستلام: 1,500 ج.م                       │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │           [      ✅ تأكيد التسليم      ]                │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌───────────────────┐    ┌───────────────────┐                │
│  │   ⚠️ مشكلة       │    │   ❌ فشل التوصيل  │                │
│  └───────────────────┘    └───────────────────┘                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**شاشة تأكيد التسليم:**
```
┌─────────────────────────────────────────────────────────────────┐
│  ← رجوع              تأكيد التسليم                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  📸 صورة إثبات التسليم *                                        │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │                    [التقط صورة]                         │   │
│  │                         📷                              │   │
│  │                                                         │   │
│  │            صورة الطرد مع العميل أو الباب               │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ✍️ توقيع العميل (اختياري)                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │                    [التقط التوقيع]                      │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  💰 تحصيل COD                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  المبلغ المطلوب: 1,500 ج.م                              │   │
│  │                                                         │   │
│  │  ☑️ تم تحصيل المبلغ كاملاً                              │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  📝 ملاحظات (اختياري)                                           │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  تم التسليم للزوجة                                      │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │           [      ✅ تأكيد وإنهاء      ]                 │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

#### UI-CD-002: شاشة التتبع للعميل

```
┌─────────────────────────────────────────────────────────────────┐
│  ← رجوع          تتبع الشحنة #CD-2026-12345                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │                    [خريطة تفاعلية]                      │   │
│  │                                                         │   │
│  │        🏪 ──────── 🚗 ─────────────► 🏠                 │   │
│  │      المتجر      السائق              أنت               │   │
│  │                                                         │   │
│  │                    📍 السائق هنا                        │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  ⏱️ الوصول المتوقع                                      │   │
│  │                                                         │   │
│  │            15-20 دقيقة                                  │   │
│  │            (2.5 كم متبقي)                               │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  👤 السائق                                                      │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  [صورة]  محمد أحمد                                      │   │
│  │          ⭐ 4.9 (523 رحلة)                               │   │
│  │          🏍️ موتوسيكل - أبيض                             │   │
│  │                                                         │   │
│  │  ┌─────────────┐    ┌─────────────┐                    │   │
│  │  │  📞 اتصال   │    │  💬 رسالة   │                    │   │
│  │  └─────────────┘    └─────────────┘                    │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  📍 رحلة الشحنة                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  ✓ تم إنشاء الطلب          10:30 ص                     │   │
│  │  │                                                      │   │
│  │  ✓ تم تعيين سائق           10:32 ص                     │   │
│  │  │                                                      │   │
│  │  ✓ تم استلام الشحنة         10:45 ص                     │   │
│  │  │                                                      │   │
│  │  ● جاري التوصيل            11:00 ص ← الآن              │   │
│  │  │                                                      │   │
│  │  ○ تم التسليم              ~11:20 ص                    │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  💰 للدفع عند الاستلام: 1,500 ج.م                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 3. قاعدة البيانات (Database Schema)

```sql
-- =====================================================
-- Crowdsourced Delivery System Database Schema
-- =====================================================

-- 1. السائقين
CREATE TABLE crowd_drivers (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT UNIQUE NOT NULL,

    -- المعلومات الشخصية
    full_name VARCHAR(100) NOT NULL,
    full_name_ar VARCHAR(100),
    phone VARCHAR(20) NOT NULL,
    email VARCHAR(255),
    date_of_birth DATE,
    gender ENUM('male', 'female') DEFAULT 'male',
    profile_photo VARCHAR(500),

    -- الهوية
    national_id VARCHAR(20) NOT NULL UNIQUE,
    national_id_front_image VARCHAR(500),
    national_id_back_image VARCHAR(500),
    national_id_verified BOOLEAN DEFAULT FALSE,
    selfie_image VARCHAR(500),
    selfie_verified BOOLEAN DEFAULT FALSE,

    -- KYC
    kyc_level ENUM('none', 'level1', 'level2', 'level3') DEFAULT 'none',
    kyc_verified_at DATETIME,
    kyc_verified_by INT,
    kyc_rejection_reason TEXT,

    -- المركبة
    vehicle_type ENUM('bicycle', 'motorcycle', 'car', 'van', 'truck') NOT NULL,
    vehicle_brand VARCHAR(50),
    vehicle_model VARCHAR(50),
    vehicle_year INT,
    vehicle_color VARCHAR(30),
    vehicle_plate VARCHAR(20),
    vehicle_photo VARCHAR(500),
    driving_license_number VARCHAR(50),
    driving_license_image VARCHAR(500),
    driving_license_expiry DATE,

    -- الموقع والتوفر
    current_lat DECIMAL(10, 8),
    current_lng DECIMAL(11, 8),
    current_heading INT,
    current_speed DECIMAL(5, 2),
    last_location_update DATETIME,
    is_online BOOLEAN DEFAULT FALSE,
    is_available BOOLEAN DEFAULT TRUE,
    current_delivery_id INT,

    -- التفضيلات
    preferred_areas JSON,
    preferred_package_types JSON,
    max_delivery_distance_km INT DEFAULT 20,
    work_hours JSON,

    -- الإحصائيات
    total_deliveries INT DEFAULT 0,
    successful_deliveries INT DEFAULT 0,
    cancelled_deliveries INT DEFAULT 0,
    failed_deliveries INT DEFAULT 0,
    average_rating DECIMAL(3, 2) DEFAULT 5.00,
    total_ratings INT DEFAULT 0,
    total_distance_km DECIMAL(10, 2) DEFAULT 0,

    -- المالية
    wallet_balance DECIMAL(10, 2) DEFAULT 0,
    pending_balance DECIMAL(10, 2) DEFAULT 0,
    total_earnings DECIMAL(12, 2) DEFAULT 0,
    total_payouts DECIMAL(12, 2) DEFAULT 0,
    pending_cod DECIMAL(10, 2) DEFAULT 0,

    -- الحساب البنكي
    payout_method ENUM('bank_transfer', 'vodafone_cash', 'etisalat_cash', 'orange_money', 'instapay'),
    bank_name VARCHAR(100),
    bank_account_number VARCHAR(50),
    bank_account_holder VARCHAR(100),
    mobile_wallet_number VARCHAR(20),

    -- الحالة
    status ENUM('pending_verification', 'active', 'suspended', 'banned', 'inactive') DEFAULT 'pending_verification',
    status_reason TEXT,
    suspended_until DATETIME,

    -- الشارات والمستوى
    driver_level ENUM('beginner', 'regular', 'experienced', 'expert', 'master') DEFAULT 'beginner',
    badges JSON,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (user_id) REFERENCES users(id),
    INDEX idx_status (status),
    INDEX idx_location (current_lat, current_lng),
    INDEX idx_available (is_online, is_available),
    INDEX idx_vehicle (vehicle_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 2. طلبات التوصيل
CREATE TABLE crowd_delivery_requests (
    id INT AUTO_INCREMENT PRIMARY KEY,
    request_code VARCHAR(20) UNIQUE NOT NULL,

    -- المصدر
    source ENUM('order', 'merchant', 'individual') NOT NULL,
    source_order_id INT,
    merchant_id INT,
    customer_id INT,

    -- الاستلام
    pickup_address TEXT NOT NULL,
    pickup_address_ar TEXT,
    pickup_lat DECIMAL(10, 8) NOT NULL,
    pickup_lng DECIMAL(11, 8) NOT NULL,
    pickup_contact_name VARCHAR(100) NOT NULL,
    pickup_contact_phone VARCHAR(20) NOT NULL,
    pickup_notes TEXT,
    pickup_time_from DATETIME,
    pickup_time_to DATETIME,

    -- التسليم
    delivery_address TEXT NOT NULL,
    delivery_address_ar TEXT,
    delivery_lat DECIMAL(10, 8) NOT NULL,
    delivery_lng DECIMAL(11, 8) NOT NULL,
    delivery_governorate VARCHAR(50) NOT NULL,
    delivery_city VARCHAR(100) NOT NULL,
    delivery_contact_name VARCHAR(100) NOT NULL,
    delivery_contact_phone VARCHAR(20) NOT NULL,
    delivery_notes TEXT,
    delivery_time_from DATETIME,
    delivery_time_to DATETIME,

    -- الشحنة
    package_type ENUM('document', 'small_package', 'medium_package', 'large_package', 'fragile', 'food') NOT NULL,
    package_weight_kg DECIMAL(5, 2),
    package_length_cm INT,
    package_width_cm INT,
    package_height_cm INT,
    package_description TEXT,
    package_value DECIMAL(10, 2),
    requires_signature BOOLEAN DEFAULT FALSE,
    is_fragile BOOLEAN DEFAULT FALSE,
    temperature_controlled BOOLEAN DEFAULT FALSE,

    -- الخيارات
    is_express BOOLEAN DEFAULT FALSE,
    is_scheduled BOOLEAN DEFAULT FALSE,
    scheduled_date DATE,
    scheduled_time_slot VARCHAR(20),

    -- التكلفة
    distance_km DECIMAL(6, 2) NOT NULL,
    estimated_duration_minutes INT NOT NULL,
    base_fee DECIMAL(10, 2) NOT NULL,
    distance_fee DECIMAL(10, 2) NOT NULL,
    additional_fees DECIMAL(10, 2) DEFAULT 0,
    express_fee DECIMAL(10, 2) DEFAULT 0,
    insurance_fee DECIMAL(10, 2) DEFAULT 0,
    total_fee DECIMAL(10, 2) NOT NULL,
    platform_fee DECIMAL(10, 2) NOT NULL,
    driver_payout DECIMAL(10, 2) NOT NULL,

    -- COD
    cod_amount DECIMAL(10, 2) DEFAULT 0,
    cod_collected BOOLEAN DEFAULT FALSE,
    cod_collected_at DATETIME,
    cod_remitted BOOLEAN DEFAULT FALSE,
    cod_remitted_at DATETIME,

    -- الحالة
    status ENUM(
        'pending',
        'finding_driver',
        'driver_assigned',
        'driver_heading_pickup',
        'arrived_pickup',
        'picked_up',
        'in_transit',
        'arrived_delivery',
        'delivered',
        'completed',
        'cancelled',
        'failed'
    ) DEFAULT 'pending',

    -- السائق
    assigned_driver_id INT,
    driver_assigned_at DATETIME,
    assignment_attempts INT DEFAULT 0,

    -- التتبع
    current_lat DECIMAL(10, 8),
    current_lng DECIMAL(11, 8),
    last_tracking_update DATETIME,

    -- التوقيتات الفعلية
    driver_accepted_at DATETIME,
    arrived_pickup_at DATETIME,
    picked_up_at DATETIME,
    in_transit_at DATETIME,
    arrived_delivery_at DATETIME,
    delivered_at DATETIME,
    completed_at DATETIME,
    cancelled_at DATETIME,

    -- الإلغاء/الفشل
    cancelled_by ENUM('customer', 'driver', 'merchant', 'system'),
    cancellation_reason TEXT,
    failure_reason TEXT,
    failure_proof_image VARCHAR(500),

    -- إثبات التسليم
    delivery_proof_image VARCHAR(500),
    recipient_signature VARCHAR(500),
    recipient_name VARCHAR(100),
    delivery_notes_driver TEXT,

    -- التقييم
    customer_rating INT,
    customer_review TEXT,
    driver_rating INT,
    driver_review TEXT,

    -- البقشيش
    tip_amount DECIMAL(10, 2) DEFAULT 0,
    tip_paid_at DATETIME,

    -- الدفع
    payment_status ENUM('pending', 'paid', 'refunded', 'failed') DEFAULT 'pending',
    payment_method VARCHAR(50),
    payment_reference VARCHAR(100),
    paid_at DATETIME,

    created_by INT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (assigned_driver_id) REFERENCES crowd_drivers(id),
    FOREIGN KEY (source_order_id) REFERENCES orders(id),
    INDEX idx_status (status),
    INDEX idx_driver (assigned_driver_id),
    INDEX idx_request_code (request_code),
    INDEX idx_created (created_at),
    INDEX idx_pickup_location (pickup_lat, pickup_lng),
    INDEX idx_delivery_location (delivery_lat, delivery_lng)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 3. سجل تتبع الموقع
CREATE TABLE crowd_delivery_tracking (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    request_id INT NOT NULL,
    driver_id INT NOT NULL,

    lat DECIMAL(10, 8) NOT NULL,
    lng DECIMAL(11, 8) NOT NULL,
    accuracy_meters INT,
    speed_kmh DECIMAL(5, 2),
    heading INT,
    altitude_meters INT,
    battery_level INT,

    recorded_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (request_id) REFERENCES crowd_delivery_requests(id),
    FOREIGN KEY (driver_id) REFERENCES crowd_drivers(id),
    INDEX idx_request_time (request_id, recorded_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 4. معاملات محفظة السائق
CREATE TABLE crowd_driver_wallet_transactions (
    id INT AUTO_INCREMENT PRIMARY KEY,
    driver_id INT NOT NULL,

    transaction_type ENUM(
        'delivery_earning',
        'tip',
        'bonus',
        'penalty',
        'adjustment',
        'payout',
        'cod_collected',
        'cod_remitted'
    ) NOT NULL,

    amount DECIMAL(10, 2) NOT NULL,
    balance_before DECIMAL(10, 2) NOT NULL,
    balance_after DECIMAL(10, 2) NOT NULL,

    related_request_id INT,
    description TEXT,
    description_ar TEXT,

    -- للسحب
    payout_method ENUM('bank_transfer', 'vodafone_cash', 'etisalat_cash', 'orange_money', 'instapay'),
    payout_account VARCHAR(100),
    payout_reference VARCHAR(100),
    payout_status ENUM('pending', 'processing', 'completed', 'failed'),
    payout_completed_at DATETIME,
    payout_failure_reason TEXT,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (driver_id) REFERENCES crowd_drivers(id),
    FOREIGN KEY (related_request_id) REFERENCES crowd_delivery_requests(id),
    INDEX idx_driver (driver_id),
    INDEX idx_type (transaction_type),
    INDEX idx_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 5. مناطق التسعير
CREATE TABLE crowd_delivery_zones (
    id INT AUTO_INCREMENT PRIMARY KEY,
    zone_code VARCHAR(20) UNIQUE NOT NULL,
    zone_name VARCHAR(100) NOT NULL,
    zone_name_ar VARCHAR(100),
    governorate VARCHAR(50) NOT NULL,
    cities JSON,

    -- التسعير
    base_price DECIMAL(10, 2) NOT NULL,
    price_per_km DECIMAL(5, 2) NOT NULL,
    min_price DECIMAL(10, 2) NOT NULL,
    max_price DECIMAL(10, 2),

    -- المضاعفات
    express_multiplier DECIMAL(3, 2) DEFAULT 1.5,
    peak_hour_multiplier DECIMAL(3, 2) DEFAULT 1.3,
    night_multiplier DECIMAL(3, 2) DEFAULT 1.2,
    weekend_multiplier DECIMAL(3, 2) DEFAULT 1.1,
    rain_multiplier DECIMAL(3, 2) DEFAULT 1.4,

    -- ساعات الذروة
    peak_hours JSON,
    night_hours JSON,

    -- الحالة
    is_active BOOLEAN DEFAULT TRUE,
    coverage_polygon JSON,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    INDEX idx_governorate (governorate),
    INDEX idx_active (is_active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 6. التقييمات
CREATE TABLE crowd_delivery_ratings (
    id INT AUTO_INCREMENT PRIMARY KEY,
    request_id INT UNIQUE NOT NULL,

    -- تقييم العميل للسائق
    customer_to_driver_rating INT CHECK (customer_to_driver_rating BETWEEN 1 AND 5),
    customer_to_driver_tags JSON,
    customer_to_driver_comment TEXT,
    customer_rated_at DATETIME,

    -- تقييم السائق للعميل
    driver_to_customer_rating INT CHECK (driver_to_customer_rating BETWEEN 1 AND 5),
    driver_to_customer_tags JSON,
    driver_to_customer_comment TEXT,
    driver_rated_at DATETIME,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (request_id) REFERENCES crowd_delivery_requests(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 7. حوافز ومكافآت السائقين
CREATE TABLE crowd_driver_incentives (
    id INT AUTO_INCREMENT PRIMARY KEY,

    incentive_code VARCHAR(20) UNIQUE NOT NULL,
    title VARCHAR(100) NOT NULL,
    title_ar VARCHAR(100),
    description TEXT,
    description_ar TEXT,

    incentive_type ENUM('per_delivery', 'milestone', 'streak', 'peak_hour', 'area_bonus', 'referral') NOT NULL,

    -- الشروط
    conditions JSON,
    /*
    Examples:
    {"min_deliveries": 10, "period": "day"}
    {"area": "new_cairo", "hours": "12:00-14:00"}
    {"streak_days": 7}
    */

    -- المكافأة
    reward_type ENUM('fixed', 'percentage', 'multiplier') NOT NULL,
    reward_value DECIMAL(10, 2) NOT NULL,

    -- الفترة
    start_date DATE NOT NULL,
    end_date DATE,

    -- الحدود
    max_uses_per_driver INT,
    max_total_uses INT,
    current_uses INT DEFAULT 0,

    is_active BOOLEAN DEFAULT TRUE,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```

---

## 4. واجهات برمجة التطبيقات (API Specification)

### 4.1 Driver Endpoints

```typescript
// Driver Registration & Profile
POST   /api/drivers/register
GET    /api/drivers/me
PUT    /api/drivers/me
POST   /api/drivers/me/documents
GET    /api/drivers/me/kyc-status

// Availability & Location
POST   /api/drivers/go-online
POST   /api/drivers/go-offline
POST   /api/drivers/location
GET    /api/drivers/me/status

// Deliveries
GET    /api/drivers/nearby-requests
POST   /api/drivers/requests/:id/accept
POST   /api/drivers/requests/:id/reject
GET    /api/drivers/active-delivery
POST   /api/drivers/deliveries/:id/status
POST   /api/drivers/deliveries/:id/pickup-confirm
POST   /api/drivers/deliveries/:id/delivery-confirm
POST   /api/drivers/deliveries/:id/report-issue

// Wallet
GET    /api/drivers/wallet
GET    /api/drivers/wallet/transactions
POST   /api/drivers/wallet/payout
GET    /api/drivers/earnings/summary

// Ratings & Stats
GET    /api/drivers/me/ratings
GET    /api/drivers/me/stats
```

### 4.2 Customer/Merchant Endpoints

```typescript
// Delivery Requests
POST   /api/deliveries/calculate-fee
POST   /api/deliveries/create
GET    /api/deliveries/:code/track
POST   /api/deliveries/:id/cancel
POST   /api/deliveries/:id/rate

// History
GET    /api/deliveries/history
```

---

## 5. الأمان والامتثال (Security & Compliance)

### 5.1 حماية البيانات
- تشفير البيانات الحساسة (AES-256)
- إخفاء أرقام الهواتف جزئياً
- حذف بيانات الموقع بعد 30 يوم

### 5.2 التحقق من الهوية
- OCR للبطاقة القومية
- Face matching بين الصورة الشخصية والبطاقة
- التحقق من رقم الهاتف (OTP)

### 5.3 أمان المعاملات المالية
- PCI DSS compliance
- Tokenization للبطاقات
- 2FA للسحوبات الكبيرة

---

## 6. الجدول الزمني (Timeline)

| الأسبوع | المهام |
|---------|--------|
| 1-2 | Database Schema + Driver Registration |
| 3-4 | Location Services + Matching Algorithm |
| 5-6 | Order Flow + Status Management |
| 7-8 | Live Tracking + Maps Integration |
| 9-10 | Payment & Wallet System |
| 11-12 | Driver App (React Native) |
| 13-14 | Customer Tracking Interface |
| 15-16 | Testing + Performance Optimization |

---

## 7. معايير النجاح (Success Metrics)

| المقياس | الهدف (شهر 3) | الهدف (شهر 6) |
|---------|---------------|---------------|
| سائقين مسجلين | 500 | 2000 |
| توصيلات يومية | 100 | 1000 |
| متوسط وقت التعيين | < 5 دقائق | < 3 دقائق |
| نسبة نجاح التوصيل | 95% | 98% |
| تقييم السائقين | 4.5+ | 4.7+ |
| رضا العملاء (NPS) | 40+ | 60+ |

---

**نهاية المستند**
