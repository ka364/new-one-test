...
import { 
  ...
  ethicalRules, InsertEthicalRule, EthicalRule,
  auditTrail, InsertAuditTrail, AuditTrail,
  events, InsertEvent, Event,
  notifications, InsertNotification, Notification,
  reports, InsertReport, Report,
  subscriptions, InsertSubscription, Subscription,
  campaigns, InsertCampaign, Campaign,
  agentInsights, InsertAgentInsight, AgentInsight,
...
export async function getActiveEthicalRules() {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(ethicalRules)
    .where(eq(ethicalRules.isActive, true))
...
export async function updateEthicalRuleStatus(ruleId: number, isActive: boolean) {
  const db = await getDb();
  if (!db) return;
  await db.update(ethicalRules).set({ isActive }).where(eq(ethicalRules.id, ruleId));
}
...
export async function updateEventStatus(eventId: number, status: Event["status"], errorMessage?: string) {
  const db = await getDb();
  if (!db) return;
...
export async function getAgentInsightsByType(agentType: AgentInsight["agentType"]) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(agentInsights)
...
export async function updateShipmentStatus(id: number, status: any, updates?: any) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(shipments).set({ status, ...updates }).where(eq(shipments.id, id));
}
...
export async function getTotalCollectionByDate(date: string) {
  ...
  const result = await db.select({ totalCash: sum(collections.amount) })
    .from(collections)
    .where(and(
  ...
  const bankResult = await db.select({ totalBank: sum(collections.amount) })
    .from(collections)
    .where(and(
      eq(collections.collectionDate, date),
...
export async function getOrCreateDailyMetrics(date: string) {
  ...
  const [existing] = await db.select().from(dailyOperationalMetrics).where(eq(dailyOperationalMetrics.date, date));
  
  if (existing) return existing;
  
  await db.insert(dailyOperationalMetrics).values({ date });
  const [newMetrics] = await db.select().from(dailyOperationalMetrics).where(eq(dailyOperationalMetrics.date, date));
...
export async function calculateAndUpdateKPIs(date: string) {
  ...
  await db.update(dailyOperationalMetrics).set({
    tcr: tcr.toFixed(2),
    tcc: tcc.toFixed(2),
    tcs: tcs.toFixed(2),
    tcrn: tcrn.toFixed(2),
...
export async function getMetricsByDateRange(startDate: string, endDate: string) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(dailyOperationalMetrics)
    .where(and(
...
export async function getAdCampaignsByDate(date: string) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(adCampaignPerformance)
...
export async function getLastCampaignEfficiency() {
  ...
  const [lastCampaign] = await db.select().from(adCampaignPerformance)
...
export async function updateActualRevenue(date: string, actualRevenue: string) {
  ...
  const [forecast] = await db.select().from(revenueForecasts).where(eq(revenueForecasts.date, date));
  ...
  await db.update(revenueForecasts).set({
    actualRevenue,
