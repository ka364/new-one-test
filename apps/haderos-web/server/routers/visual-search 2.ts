...
export const visualSearchRouter = router({
  ...
  searchByImage: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const { url: searchImageUrl } = await storagePut(searchImageKey, imageBuffer, 'image/jpeg');

        // 2. Get all product embeddings from database
        const allEmbeddings = await getAllImageEmbeddings();

        // 3. Perform visual search
        const searchResult = await analyzeAndSearchImage(searchImageUrl, allEmbeddings, {
          topK: input.topK,
  ...
  provideFeedback: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      return await updateSearchFeedback(input.searchId, input.wasHelpful, input.selectedProductId);
    }),
});
