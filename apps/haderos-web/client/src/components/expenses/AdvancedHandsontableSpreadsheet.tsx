...
import { 
  Loader2, Save, Upload, Download, RefreshCw, MessageSquare, 
  Clock, Share2, BarChart3, Plus, Undo, Redo, FileText,
  Users, Eye, Edit, Trash2, CheckCircle, XCircle, AlertTriangle
} from "lucide-react";
...
export function AdvancedHandsontableSpreadsheet({ 
  ...
  const { data: commentsData } = trpc.spreadsheet.getComments.useQuery({ sessionId: sessionId || '' }, {
    enabled: !!sessionId,
  });
  
  const { data: versionsData } = trpc.spreadsheet.getVersions.useQuery({ sessionId: sessionId || '' }, {
    enabled: !!sessionId,
  });
  
  const { data: sharingData } = trpc.spreadsheet.getSharing.useQuery({ sessionId: sessionId || '' }, {
    enabled: !!sessionId,
  });
  ...
  const handleAddComment = async () => {
    ...
    const columnKeys = ['title', 'amount', 'currency', 'expenseDate', 'category', 'expenseType', 'status'];
  ...
  const cellRenderer = useCallback((instance: any, td: HTMLTableCellElement, row: number, col: number, prop: string, value: any, cellProperties: any) => {
        ...
        <CardContent>
          {pendingChanges.length > 0 && (
            <Alert className="mb-4">
              <AlertDescription>
                ⚠️ لديك {pendingChanges.length} تغيير غير محفوظ
              </AlertDescription>
            </Alert>
          )}
