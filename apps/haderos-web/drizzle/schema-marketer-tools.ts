...
import { pgTable, serial, varchar, text, timestamp, decimal, integer, jsonb, boolean, date, index, unique } from "drizzle-orm/pg-core";
...
export const marketerAccounts = pgTable("marketer_accounts", {
...
export const marketerLandingPages = pgTable("marketer_landing_pages", {
  id: serial("id").primaryKey(),
  marketerId: integer("marketer_id").notNull().references(() => marketerAccounts.id),
  ...
  content: jsonb("content").$type<{
    ...
    sections: Array<{
      id: string;
      type: 'features' | 'benefits' | 'testimonials' | 'pricing' | 'faq' | 'video' | 'gallery' | 'countdown' | 'custom';
      title?: string;
      titleAr?: string;
      content: any;
      order: number;
...
export const marketerWebsites = pgTable("marketer_websites", {
  id: serial("id").primaryKey(),
  marketerId: integer("marketer_id").notNull().references(() => marketerAccounts.id),
...
export const marketerShopifyStores = pgTable("marketer_shopify_stores", {
  id: serial("id").primaryKey(),
  marketerId: integer("marketer_id").notNull().references(() => marketerAccounts.id),
...
export const adCampaignTemplates = pgTable("ad_campaign_templates", {
  id: serial("id").primaryKey(),
  ...
  // Content Templates
  headlines: jsonb("headlines").$type<Array<{
    text: string;
    textAr?: string;
    charLimit: number;
  }>>().notNull(),

  descriptions: jsonb("descriptions").$type<Array<{
    text: string;
    textAr?: string;
    charLimit: number;
  }>>().notNull(),

  callToActions: jsonb("call_to_actions").$type<Array<{
    text: string;
    textAr?: string;
    type: string;
  }>>().notNull(),
...
export const marketerLeads = pgTable("marketer_leads", {
  id: serial("id").primaryKey(),
  marketerId: integer("marketer_id").notNull().references(() => marketerAccounts.id),
...
export const marketerAnalytics = pgTable("marketer_analytics", {
  id: serial("id").primaryKey(),
  marketerId: integer("marketer_id").notNull().references(() => marketerAccounts.id),
...
export const DEFAULT_AD_TEMPLATES = [
  {
    ...
    nameAr: "إطلاق منتج",
    headlines: [
      { text: "🔥 New Arrival - Limited Stock!", textAr: "🔥 وصل حديثاً - كمية محدودة!", charLimit: 40 },
      { text: "Get Yours Before It's Gone!", textAr: "احصل عليه قبل نفاد الكمية!", charLimit: 40 }
    ],
    descriptions: [
      { text: "High quality, factory direct prices. Order now!", textAr: "جودة عالية، أسعار المصنع مباشرة. اطلب الآن!", charLimit: 125 }
    ],
    callToActions: [
  ...
  },
  {
    ...
    nameAr: "إعلان ستوري",
    headlines: [
      { text: "✨ Swipe Up for Exclusive Deals!", textAr: "✨ اسحب للأعلى للعروض الحصرية!", charLimit: 30 }
    ],
    descriptions: [
      { text: "Premium quality at unbeatable prices", textAr: "جودة متميزة بأسعار لا تُقاوم", charLimit: 90 }
    ],
    callToActions: [
      { text: "Swipe Up", textAr: "اسحب للأعلى", type: "SWIPE_UP" }
    ]
  },
  {
    platform: "tiktok",
    ...
    nameAr: "عرض منتج",
    headlines: [
      { text: "This is going viral! 🔥", textAr: "هذا ينتشر بسرعة! 🔥", charLimit: 25 }
    ],
    descriptions: [
      { text: "Factory direct | Best quality | Fast shipping", textAr: "من المصنع مباشرة | أفضل جودة | شحن سريع", charLimit: 80 }
    ],
    callToActions: [
      { text: "Shop Now", textAr: "تسوق الآن", type: "SHOP_NOW" }
    ]
