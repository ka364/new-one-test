...
export const employees = pgTable(
  ...
  (table) => [index('national_id').on(table.nationalId)]
);
...
export const founderAccounts = pgTable(
  'founder_accounts',
  {
    id: serial().notNull(),
    fullName: varchar('full_name', { length: 255 }).notNull(),
  ...
  (table) => [index('email').on(table.email), index('username').on(table.username)]
);
...
export const monthlyEmployeeAccounts = pgTable(
  ...
  (table) => [index('monthly_emp_accounts_username').on(table.username)]
);
...
export const orders = pgTable(
  ...
  (table) => [index('orders_orderNumber_unique').on(table.orderNumber)]
);
...
export const productImageRequests = pgTable(
  ...
  (table) => [index('product_image_requests_requestNumber_unique').on(table.requestNumber)]
);
...
export const products = pgTable(
  ...
  (table) => [index('products_model_code_unique').on(table.modelCode)]
);
...
export const shopifyOrders = pgTable(
  ...
  (table) => [index('shopify_order_id').on(table.shopifyOrderId)]
);
...
export const shopifyProducts = pgTable(
  ...
  (table) => [index('shopify_product_id').on(table.shopifyProductId)]
);
...
export const shopifyVariants = pgTable(
  ...
  (table) => [index('shopify_variant_id').on(table.shopifyVariantId)]
);
...
export const subscriptions = pgTable(
  ...
  (table) => [index('subscriptions_subscriptionNumber_unique').on(table.subscriptionNumber)]
);
...
export const transactions = pgTable(
  ...
  (table) => [index('transactions_transactionNumber_unique').on(table.transactionNumber)]
);
...
export const investors = pgTable(
  ...
  (table) => [index('investors_email_unique').on(table.email)]
);

export const investorActivity = pgTable(
  'investor_activity',
  {
    id: serial().notNull(),
    investorId: integer()
...
export const visualSearchHistory = pgTable(
...
export const shippingCompanies = pgTable('shipping_companies', {
  ...
  returnFeePercentage: decimal('return_fee_percentage', { precision: 5, scale: 2 }).default(
    '40.00'
  ),
  exchangeFeePercentage: decimal('exchange_fee_percentage', { precision: 5, scale: 2 }).default(
    '150.00'
  ),
...
export const dailyOperationalMetrics = pgTable('daily_operational_metrics', {
  ...
  ordersCreatedValue: decimal('orders_created_value', { precision: 10, scale: 2 })
    .default('0.00')
    .notNull(),
  ordersConfirmed: integer('orders_confirmed').default(0).notNull(),
  ordersConfirmedValue: decimal('orders_confirmed_value', { precision: 10, scale: 2 })
    .default('0.00')
    .notNull(),
  ordersShipped: integer('orders_shipped').default(0).notNull(),
  ordersShippedValue: decimal('orders_shipped_value', { precision: 10, scale: 2 })
    .default('0.00')
    .notNull(),
  ordersReturned: integer('orders_returned').default(0).notNull(),
  ordersReturnedValue: decimal('orders_returned_value', { precision: 10, scale: 2 })
    .default('0.00')
    .notNull(),
  ordersDelivered: integer('orders_delivered').default(0).notNull(),
  ordersDeliveredValue: decimal('orders_delivered_value', { precision: 10, scale: 2 })
    .default('0.00')
    .notNull(),

  // Collections
  totalCollection: decimal('total_collection', { precision: 10, scale: 2 })
    .default('0.00')
    .notNull(),
...
export const revenueForecasts = pgTable('revenue_forecasts', {
  ...
  lastCampaignEfficiency: decimal('last_campaign_efficiency', {
    precision: 10,
    scale: 4,
  }).notNull(), // Cost per result
...
// 4. جدول سجل التتبع
export const trackingLogs = pgTable(
  'tracking_logs',
  {
    id: serial().primaryKey(),
    codOrderId: integer('cod_order_id').references(() => codOrders.id),
...
export const egyptCenters = pgTable('egypt_centers', {
  ...
  nameEn: varchar('name_en', { length: 100 }).notNull(),
  nameAr: varchar('name_ar', { length: 100 }).notNull(),
  governorateCode: varchar('governorate_code', { length: 10 }).references(
    () => egyptGovernorates.code
  ),
...
export const shippingPerformanceByPoint = pgTable(
  'shipping_performance_by_point',
  {
    id: serial().primaryKey(),
    companyId: integer('company_id')
      .references(() => shippingPartners.id)
      .notNull(),
    pointCode: varchar('point_code', { length: 15 })
      .references(() => egyptCities.code)
      .notNull(),
    ...
    customerSatisfaction: decimal('customer_satisfaction', { precision: 3, scale: 2 }).default(
      '0.00'
    ),
  ...
  (table) => [index('perf_point_company_point').on(table.companyId, table.pointCode)]
);

// Level 2: Performance by Center (fallback 1)
export const shippingPerformanceByCenter = pgTable(
  ...
  (table) => [index('perf_center_company_center').on(table.companyId, table.centerCode)]
);

// Level 3: Performance by Governorate (fallback 2)
export const shippingPerformanceByGovernorate = pgTable(
  'shipping_performance_by_governorate',
  {
    ...
    companyId: integer('company_id')
      ...
      .notNull(),
    governorateCode: varchar('governorate_code', { length: 10 })
      .references(() => egyptGovernorates.code)
      .notNull(),
    governorateName: varchar('governorate_name', { length: 100 }),
  ...
  (table) => [index('perf_gov_company_gov').on(table.companyId, table.governorateCode)]
);
